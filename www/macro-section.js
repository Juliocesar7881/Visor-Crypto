...
(function() {
    ...
    const ACTIVE_WORKER_URL = (() => {
        try { return String(localStorage.getItem('vc_active_worker_url_v1') || '').trim().replace(/\/+$/, ''); } catch (_) { return ''; }
    })();
    const CALENDAR_WORKER_URLS = [...new Set([ACTIVE_WORKER_URL, ...(
        typeof window.getVisorWorkerUrls === 'function'
        ? window.getVisorWorkerUrls()
        : [...new Set([
            APP_CONFIG.CALENDAR_WORKER_URL,
            APP_CONFIG.CALENDAR_WORKER_FALLBACK_URL
        ].map(url => String(url || '').trim().replace(/\/+$/, '')).filter(Boolean))]
    )].filter(Boolean))];
    ...
    const MARKET_INDICATORS = {
        ...
        '^GSPC': { name: 'S&P 500', short: 'SPX', desc: 'S&P 500 Index', img: 'S&P500.png', color: '#ef4444', prefix: '', decimals: 2 },
        ...
        '^VIX': { name: 'VIX', short: 'VIX', desc: 'Índice de Volatilidade S&P 500', img: 'VIX.png', color: '#22c55e', prefix: '', decimals: 2 },
        'XLE': { name: 'Energia', short: 'XLE', desc: 'Energy Select Sector SPDR', img: 'XLE.png', color: '#3b82f6', prefix: '$', decimals: 2 },
    };
    ...
    function getMacroWorkerMarketUrl(path) {
        const workerUrl = CALENDAR_WORKER_URLS[0] || CALENDAR_WORKER_URL;
        if (!workerUrl) return '';
    ...
    async function loadPricesViaWorkerYahooQuotes(symbols, options = {}) {
        const { timeoutMs = 1800, onSymbolUpdate = null } = options;
        const safeSymbols = [...new Set((symbols || []).filter(Boolean))];
        if (!safeSymbols.length) return 0;

        const workerUrl = getMacroWorkerMarketUrl('/market/yahoo-quotes?symbols=' + encodeURIComponent(safeSymbols.join(',')));
        if (!workerUrl) return 0;
        ...
        try {
            ...
            let loaded = 0;
            items.forEach((item) => {
                if (applyYahooBatchQuoteItem(item, { onSymbolUpdate })) loaded++;
            });
            return loaded;
        } catch (_) {
            return 0;
    ...
    async function loadPricesViaYahooQuoteBatch(options = {}) {
        ...
        const workerLoaded = await loadPricesViaWorkerMacroQuotes({ timeoutMs, onSymbolUpdate });
        if (workerLoaded > 0) return workerLoaded;

        const sourceUrls = [
            `https://query1.finance.yahoo.com/v7/finance/quote?symbols=${encodeURIComponent(targetSymbols.join(','))}`,
            `https://query2.finance.yahoo.com/v7/finance/quote?symbols=${encodeURIComponent(targetSymbols.join(','))}`,
        ];
    ...
    async function loadPricesViaYahooV8(options = {}) {
        ...
        async function fetchSymbol(symbol) {
            ...
            const workerLoaded = await loadPricesViaWorkerYahooQuotes([symbol], {
    ...
    function formatIndicatorPrice(symbol) {
        ...
        const value = rawPrice.toLocaleString(window.VisorI18n?.getLocale?.() || 'en-US', { 
    ...
    function openIndicatorModal(symbol) {
        ...
        window.VisorMonetization?.recordAction('macro_indicator_open').catch(() => {});
    ...
    async function loadFullscreenChartData(symbol) {
        ...
        if (cached && (Date.now() - cached.timestamp) < CHART_CACHE_TTL) {
            ...
            updateChartDataBadge(indicatorCandleData);
    ...
    function drawFullscreenLineChart(candleData, symbol) {
        ...
        for (let i = 0; i < visibleData.length; i += Math.max(1, timeGridSpacing)) {
            ...
            if (['15m', '30m', '4h', '1d'].includes(indicatorChartPeriod)) {
                label = date.toLocaleTimeString(window.VisorI18n?.getLocale?.() || 'en-US', { hour: '2-digit', minute: '2-digit' });
            } else if (['1w', '1M'].includes(indicatorChartPeriod)) {
                label = date.toLocaleDateString(window.VisorI18n?.getLocale?.() || 'en-US', { day: '2-digit', month: '2-digit' });
            } else {
                label = date.toLocaleDateString(window.VisorI18n?.getLocale?.() || 'en-US', { month: 'short', year: '2-digit' });
            }
    ...
    const CHART_PERSIST_CACHE_KEY = 'vc_macro_chart_cache_v1';
    const CHART_PERSIST_CACHE_MAX_AGE = 24 * 60 * 60 * 1000;
    ...
    function _attachChartMeta(candles, meta = {}) {
        const list = Array.isArray(candles) ? candles : [];
        try {
            Object.defineProperty(list, '_chartMeta', {
                value: meta || {},
                enumerable: false,
                configurable: true
            });
        } catch (_) {
            list._chartMeta = meta || {};
        }
        return list;
    }

    function _normalizeChartCandles(value, period, stats = null, meta = {}) {
        const raw = Array.isArray(value) ? value : [];
        const candles = raw.map((item) => {
            if (Array.isArray(item)) {
                return [
                    Number(item[0] || 0),
                    _toFinitePositive(item[1], _toFinitePositive(item[4], 0)),
                    _toFinitePositive(item[2], _toFinitePositive(item[4], 0)),
                    _toFinitePositive(item[3], _toFinitePositive(item[4], 0)),
                    _toFinitePositive(item[4], 0),
                    Number(item[5] || 0) || 0
                ];
            }
            const close = _toFinitePositive(item?.close, 0);
            return [
                Number(item?.timestamp || item?.ts || item?.time || 0),
                _toFinitePositive(item?.open, close),
                _toFinitePositive(item?.high, close),
                _toFinitePositive(item?.low, close),
                close,
                Number(item?.volume || 0) || 0
            ];
        }).filter((item) => item[0] > 0 && item[4] > 0);

        const normalizedStats = stats || _calculateStatsFromCandles(candles);
        const selected = _attachPeriodStats(candles, normalizedStats);
        return _attachChartMeta(selected, meta);
    }

    function readPersistedChartCache(symbol, period) {
        try {
            const raw = localStorage.getItem(CHART_PERSIST_CACHE_KEY);
            if (!raw) return null;
            const parsed = JSON.parse(raw);
            const entry = parsed?.[getChartCacheKey(symbol, period)];
            if (!entry || !Array.isArray(entry.data)) return null;
            const ageMs = Date.now() - Number(entry.timestamp || 0);
            if (!Number.isFinite(ageMs) || ageMs < 0 || ageMs > CHART_PERSIST_CACHE_MAX_AGE) return null;
            return _normalizeChartCandles(entry.data, period, entry.stats || null, {
                source: entry.source || 'cache local',
                stale: true,
                updatedAt: entry.updatedAt || entry.timestamp || 0
            });
        } catch (_) {
            return null;
        }
    ...
    function writePersistedChartCache(symbol, period, candles, meta = {}) {
        try {
            if (!Array.isArray(candles) || candles.length === 0) return;
            const raw = localStorage.getItem(CHART_PERSIST_CACHE_KEY);
            const parsed = raw ? JSON.parse(raw) : {};
            const keys = Object.keys(parsed);
            if (keys.length > MAX_CHART_CACHE_ITEMS) {
                keys
                    .sort((a, b) => Number(parsed[a]?.timestamp || 0) - Number(parsed[b]?.timestamp || 0))
                    .slice(0, keys.length - MAX_CHART_CACHE_ITEMS)
                    .forEach((key) => delete parsed[key]);
            }
            parsed[getChartCacheKey(symbol, period)] = {
                data: candles.map((c) => Array.isArray(c) ? c.slice(0, 6) : c),
                stats: candles._periodStats || _calculateStatsFromCandles(candles),
                source: meta.source || candles._chartMeta?.source || 'chart',
                updatedAt: meta.updatedAt || candles._chartMeta?.updatedAt || Date.now(),
                timestamp: Date.now()
            };
            localStorage.setItem(CHART_PERSIST_CACHE_KEY, JSON.stringify(parsed));
        } catch (_) {}
    }
    ...
    function updateChartDataBadge(candles) {
        const container = document.getElementById('macro-chart-container');
        if (!container) return;
        const existing = container.querySelector('.macro-chart-data-badge');
        if (existing) existing.remove();
        const meta = candles?._chartMeta || {};
        if (!meta.stale) return;
        const badge = document.createElement('div');
        badge.className = 'macro-chart-data-badge';
        badge.style.cssText = 'position:absolute;right:10px;top:10px;z-index:4;padding:4px 8px;border-radius:999px;background:rgba(15,23,42,0.78);border:1px solid rgba(148,163,184,0.22);color:#a5b4fc;font-size:9px;font-weight:700;pointer-events:none;';
        badge.textContent = 'Último dado válido';
    ...
    async function _fetchWorkerYahooChart(symbol, period) {
        const workerUrl = getMacroWorkerMarketUrl('/market/yahoo-chart?symbol=' + encodeURIComponent(symbol) + '&period=' + encodeURIComponent(period));
        ...
        try {
            ...
            try {
                const response = await _fetchWithTimeout(workerUrl, {}, 7000);
        ...
        if (!data || data.success === false || !Array.isArray(data.candles) || data.candles.length === 0) return null;
        return _normalizeChartCandles(data.candles, period, data.stats || null, {
    ...
    function _parseYahooChartResponse(data, period) {
        const result = data?.chart?.result?.[0];
        if (!result) return [];

        const timestamps = result.timestamp;
        const q = result.indicators?.quote?.[0];
        if (!timestamps || timestamps.length === 0 || !q || !q.close) return [];
        ...
        return _attachChartMeta(_selectVisiblePeriodCandles(rawCandles, period), {
            source: 'yahoo_direct',
            stale: false,
            updatedAt: Date.now()
        });
    ...
    async function _fetchYahooChart(symbol, period) {
        const workerCandles = await _fetchWorkerYahooChart(symbol, period);
        if (workerCandles && workerCandles.length > 0) {
            writePersistedChartCache(symbol, period, workerCandles, workerCandles._chartMeta || {});
            macroLog(`Worker chart ${symbol} (${period}): ${workerCandles.length} candles`, 'success');
            return workerCandles;
        }
        ...
        for (const attempt of attempts) {
            ...
            // Tentativa 0: Native HTTP Android evita CORS no WebView.
            try {
                data = await nativeHttpGet(yahooUrl, {
                    connectTimeout: 5200,
                    readTimeout: 5200,
                    fetchTimeoutMs: 5200
                });
            ...
            try {
                if (data?.chart?.result?.[0]) throw new Error('native_ok');
            ...
            if (candles.length > 0) {
                ...
                writePersistedChartCache(symbol, period, candles, { source: 'yahoo_direct', updatedAt: Date.now() });
                return candles;
            }
        }

        const persisted = readPersistedChartCache(symbol, period);
        if (persisted && persisted.length > 0) {
            macroLog(`Grafico ${symbol} (${period}) do cache local persistente`, 'warn');
            return persisted;
        }
    ...
    function drawIndicatorLineChart(candleData, symbol = currentIndicatorSymbol) {
        ...
        if (!candleData || candleData.length === 0) {
            ...
            ctx.fillText('Dados temporariamente indisponiveis', rect.width / 2, rect.height / 2);
            return;
        }
        ...
        for (let i = 0; i < timestamps.length; i += step) {
            ...
            if (indicatorChartPeriod === '1d' || indicatorChartPeriod === '15m' || indicatorChartPeriod === '30m' || indicatorChartPeriod === '4h') {
                ...
                label = date.toLocaleDateString(window.VisorI18n?.getLocale?.() || 'en-US', { day: '2-digit', month: 'short' });
            }
    ...
    async function fetchRealTAData(symbol) {
            ...
            for (let attempt = 0; attempt < 3; attempt++) {
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        if (json.values && json.values.length >= MIN_TA_CANDLES) {
        ...
        // === Strategy 2: Worker/cache chart candles (same pipeline used by the visible chart) ===
        if (!closes || closes.length < MIN_TA_CANDLES) {
            try {
                const candles = await _fetchYahooChart(symbol, '1Y');
                if (Array.isArray(candles) && candles.length >= MIN_TA_CANDLES) {
                    const rows = candles
                        .map(c => ({
                            close: _toFinitePositive(c?.[4], 0),
                            high: _toFinitePositive(c?.[2], _toFinitePositive(c?.[4], 0)),
                            low: _toFinitePositive(c?.[3], _toFinitePositive(c?.[4], 0))
                        }))
                        .filter(c => c.close > 0 && c.high > 0 && c.low > 0);
                    if (rows.length >= MIN_TA_CANDLES) {
                        closes = rows.map(c => c.close);
                        highs = rows.map(c => c.high);
                        lows = rows.map(c => c.low);
                    }
                }
            } catch(e) {}
        }

        // === Strategy 3: Yahoo Finance direto/native HTTP ===
        if (!closes || closes.length < MIN_TA_CANDLES) {
            const yahooUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?interval=1d&range=3mo`;
            try {
                const data = await nativeHttpGet(yahooUrl, {
                    connectTimeout: 6500,
                    readTimeout: 6500,
                    fetchTimeoutMs: 6500
                });
        ...
        if (!closes || closes.length < MIN_TA_CANDLES) throw new Error('Dados insuficientes para análise');
    ...
    const FED_LAST_VALID_CACHE_KEY = 'vc_fedwatch_last_valid_v1';
    ...
    const FED_RATE_LAST_VALID_MAX_AGE = 180 * 24 * 60 * 60 * 1000;

    function _loadFedPersistedCache() {
        try {
            const keys = [FED_LAST_VALID_CACHE_KEY, FED_CACHE_PERSIST_KEY];
            for (const key of keys) {
                const raw = localStorage.getItem(key);
                if (!raw) continue;
                const parsed = JSON.parse(raw);
                if (!parsed || !parsed.currentRate) continue;
                return parsed;
            }
            return null;
        } catch (_) {
    ...
    function _saveFedPersistedCache(data) {
        try {
            const normalized = normalizeFedData(data);
            if (!hasUsableFedRateData(normalized)) return;
            const savedAt = Date.now();
            const lastUpdate = Number(normalized.lastUpdate || normalized.updatedAt || normalized.fetchedAt || savedAt);
            const payload = {
                ...normalized,
                lastUpdate: Number.isFinite(lastUpdate) && lastUpdate > 0 ? lastUpdate : savedAt,
                updatedAt: Number(normalized.updatedAt || lastUpdate || savedAt) || savedAt,
                cachedAt: savedAt,
                cacheVersion: FED_LAST_VALID_CACHE_KEY
            };
            localStorage.setItem(FED_LAST_VALID_CACHE_KEY, JSON.stringify(payload));
            localStorage.setItem(FED_CACHE_PERSIST_KEY, JSON.stringify(payload));
        } catch (_) {}
    }
    ...
    function formatFedProbabilityTimestamp(timestamp) {
        ...
        return d.toLocaleString(window.VisorI18n?.getLocale?.() || 'en-US', {
            day: '2-digit',
            month: '2-digit',
            hour: '2-digit',
            minute: '2-digit'
    ...
    const FED_PROB_RETRY_DELAY_MS = 15000;
    const FED_PROB_RETRY_MAX_DELAY_MS = 60000;
    let fedWatchRetryTimer = null;
    let fedWatchRetryAttempt = 0;

    function clearFedWatchRetry() {
        if (fedWatchRetryTimer) {
            clearTimeout(fedWatchRetryTimer);
            fedWatchRetryTimer = null;
        }
        fedWatchRetryAttempt = 0;
    }

    function scheduleFedWatchRetry(reason) {
        if (fedWatchRetryTimer) return;
        fedWatchRetryAttempt += 1;
        const delay = Math.min(FED_PROB_RETRY_MAX_DELAY_MS, FED_PROB_RETRY_DELAY_MS * fedWatchRetryAttempt);
        macroLog(`Fed Watch retry agendado (${reason || 'dados pendentes'}) em ${Math.round(delay / 1000)}s`, 'warn');
        fedWatchRetryTimer = setTimeout(() => {
            fedWatchRetryTimer = null;
            try {
                Promise.resolve(updateFedWatch({ silentRetry: true })).catch(() => {});
            } catch (_) {}
        }, delay);
    }

    function getFedWatchActionLabel(action, rate) {
        if (!rate || !Number.isFinite(rate.lower) || !Number.isFinite(rate.upper)) {
            if (action === 'cut') return 'Corte';
            if (action === 'hold') return 'Manutenção';
            return 'Aumento';
        }

        if (action === 'cut') return `Corte (${(rate.lower - 0.25).toFixed(2)}-${(rate.upper - 0.25).toFixed(2)}%)`;
        if (action === 'hold') return `Manutenção (${rate.lower.toFixed(2)}-${rate.upper.toFixed(2)}%)`;
        return `Aumento (${(rate.lower + 0.25).toFixed(2)}-${(rate.upper + 0.25).toFixed(2)}%)`;
    }

    function renderFedWatchSearching(container, fedData, options = {}) {
        if (!container) return;
        if (options.keepExisting && container.querySelector('.fed-prob-item')) return;

        const rate = fedData?.currentRate || null;
        const rateLine = rate && Number.isFinite(rate.lower) && Number.isFinite(rate.upper)
            ? `<p style="margin: 6px 0 0; font-size: 12px; color: #888;">Taxa atual: ${rate.lower.toFixed(2)}% - ${rate.upper.toFixed(2)}%</p>`
            : '';
        const message = options.message || 'Buscando probabilidades reais para a próxima reunião...';

        container.innerHTML = `
            <div style="text-align: center; padding: 22px; color: #94a3b8;">
                <i class="fas fa-spinner fa-spin" style="font-size: 22px; margin-bottom: 10px; opacity: 0.85;"></i>
                <p style="margin: 0; font-weight: 600;">Fed Watch carregando dados reais</p>
                ${rateLine}
                <p style="margin: 8px 0 0; font-size: 12px; color: #777;">${message}</p>
            </div>
        `;
    }

    function renderFedWatchUnavailable(container, fedData, options = {}) {
        if (!container) return;
        if (options.keepExisting && container.querySelector('.fed-prob-item')) return;
        const rate = fedData?.currentRate || null;
        const rateLine = rate && Number.isFinite(rate.lower) && Number.isFinite(rate.upper)
            ? `<p style="margin: 6px 0 0; font-size: 12px; color: #888;">Taxa atual: ${rate.lower.toFixed(2)}% - ${rate.upper.toFixed(2)}%</p>`
            : '';
        const message = options.message || 'A Polymarket nao tem um mercado ativo confiavel para esta reuniao neste momento.';
        container.innerHTML = `
            <div style="text-align: center; padding: 22px; color: #94a3b8;">
                <i class="fas fa-circle-info" style="font-size: 22px; margin-bottom: 10px; opacity: 0.85;"></i>
                <p style="margin: 0; font-weight: 700;">Probabilidades indisponiveis</p>
                ${rateLine}
                <p style="margin: 8px 0 0; font-size: 12px; color: #777; line-height: 1.45;">${message}</p>
            </div>
        `;
    }

    function renderFedWatchProbabilities(container, fedData, probs) {
        if (!container || !probs) return;
        const rate = fedData?.currentRate || null;
        const hasRate = !!(rate && Number.isFinite(rate.lower) && Number.isFinite(rate.upper));
        const dataSource = fedData?.dataSource || (hasRate ? 'FRED' : 'buscando taxa');
        ...
        const dffLabel = fedData?.effectiveRate ? fedData.effectiveRate.toFixed(2) + '%' : '--';
        const cpiLabel = fedData?.cpi ? fedData.cpi.toFixed(1) + '%' : '--';
        const unemploymentLabel = fedData?.unemployment ? fedData.unemployment.toFixed(1) + '%' : '--';

        container.innerHTML = `
            <div class="fed-prob-item">
                <div class="fed-prob-header">
                    <span class="fed-prob-action cut"><i class="fas fa-arrow-down"></i> ${getFedWatchActionLabel('cut', rate)}</span>
            ...
            <div class="fed-prob-item">
                <div class="fed-prob-header">
                    <span class="fed-prob-action hold"><i class="fas fa-equals"></i> ${getFedWatchActionLabel('hold', rate)}</span>
            ...
            <div class="fed-prob-item">
                <div class="fed-prob-header">
                    <span class="fed-prob-action hike"><i class="fas fa-arrow-up"></i> ${getFedWatchActionLabel('hike', rate)}</span>
            ...
            <div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid rgba(255,255,255,0.1); font-size: 10px; color: #555; display: flex; flex-direction: column; gap: 4px;">
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <span><i class="fas fa-sync-alt"></i> ${new Date().toLocaleTimeString(window.VisorI18n?.getLocale?.() || 'en-US', { hour: '2-digit', minute: '2-digit' })} BRT</span>
                ...
                <div style="font-size: 9px; color: #444;">
                    ...
                    DFF: ${dffLabel}${fedData?.obsDate?.dff ? ' (' + new Date(fedData.obsDate.dff + 'T00:00:00').toLocaleDateString(window.VisorI18n?.getLocale?.() || 'en-US', {day:'2-digit',month:'2-digit'}) + ')' : ''} | CPI: ${cpiLabel}${fedData?.obsDate?.cpi ? ' (' + new Date(fedData.obsDate.cpi + 'T00:00:00').toLocaleDateString(window.VisorI18n?.getLocale?.() || 'en-US', {day:'2-digit',month:'2-digit'}) + ')' : ''} | Desemp: ${unemploymentLabel}${fedData?.obsDate?.unrate ? ' (' + new Date(fedData.obsDate.unrate + 'T00:00:00').toLocaleDateString(window.VisorI18n?.getLocale?.() || 'en-US', {day:'2-digit',month:'2-digit'}) + ')' : ''}
                </div>
                ${probs.stale ? `
    ...
    function normalizeFedData(raw) {
        ...
        const nested = raw.fedRate && typeof raw.fedRate === 'object' ? raw.fedRate : {};
        const effectiveRate = toFiniteNumber(raw.effectiveRate != null ? raw.effectiveRate : nested.effectiveRate);
        const targetUpper = toFiniteNumber(raw.targetUpper != null ? raw.targetUpper : nested.targetUpper);
        const targetLower = toFiniteNumber(raw.targetLower != null ? raw.targetLower : nested.targetLower);
        const currentRate = normalizeRateRange(
            raw.currentRate != null ? raw.currentRate : nested.currentRate,
            targetLower,
            targetUpper,
            effectiveRate
        );
        ...
        const lastUpdate = Number(raw.lastUpdate || raw.updatedAt || raw.fetchedAt || raw.cachedAt || nested.updatedAt || Date.now());
        ...
        return {
            ...
            targetUpper,
            targetLower,
            cpi: toFiniteNumber(raw.cpi != null ? raw.cpi : nested.cpi),
            unemployment: toFiniteNumber(raw.unemployment != null ? raw.unemployment : nested.unemployment),
            currentRate,
            probabilities: normalizeFedProbabilities(raw.probabilities),
            obsDate: raw.obsDate || nested.obsDate || null,
            dataSource: raw.dataSource || raw.rateSource || nested.dataSource || nested.source || 'FRED',
    ...
    function getUsableFedRateCache(maxAgeMs = FED_RATE_LAST_VALID_MAX_AGE) {
    ...
    function rememberFedRateData(raw) {
        const normalized = normalizeFedData(raw);
        if (!hasUsableFedRateData(normalized)) return null;
        fedDataCache = {
            ...fedDataCache,
            ...normalized,
            lastUpdate: Number(normalized.lastUpdate || Date.now()) || Date.now()
        };
        _saveFedPersistedCache(fedDataCache);
        return fedDataCache;
    }

    function formatFedRateRange(rate) {
        if (!rate || !Number.isFinite(rate.lower) || !Number.isFinite(rate.upper)) return '';
        return `${rate.lower.toFixed(2)}% - ${rate.upper.toFixed(2)}%`;
    }

    function renderFedRateHeader(fedData, options = {}) {
        ...
        const normalized = normalizeFedData(fedData);
        if (hasUsableFedRateData(normalized)) {
            const rate = normalized.currentRate;
            if (currentRateEl) currentRateEl.textContent = formatFedRateRange(rate);
            if (lastDecisionEl) {
                const effectiveRate = normalized.effectiveRate || ((rate.lower + rate.upper) / 2);
                const ageMs = Date.now() - Number(normalized.lastUpdate || 0);
                const isStale = !!options.stale || (Number.isFinite(ageMs) && ageMs > FED_CACHE_TTL);
                const timestamp = formatFedProbabilityTimestamp(normalized.lastUpdate);
                const staleLine = isStale && timestamp ? `<div style="margin-top: 2px; color: var(--text-muted); font-size: 9px; font-weight: 500;">Ultima atualizacao: ${timestamp}</div>` : '';
                lastDecisionEl.innerHTML = `<span style="color: var(--accent-blue, #3b82f6); font-size: 16px; font-weight: 800;">${effectiveRate.toFixed(2)}%</span>${staleLine}`;
            }
            return true;
        }
        ...
        if (lastDecisionEl) {
            lastDecisionEl.innerHTML = '<span style="color: var(--text-muted);">--</span>';
        }
        return false;
    }
    ...
    async function fetchFedRateFromNyFed() {
        const end = new Date();
        const start = new Date(end.getTime() - 28 * 24 * 60 * 60 * 1000);
        const url = 'https://markets.newyorkfed.org/api/rates/unsecured/effr/search.json'
            + '?startDate=' + encodeURIComponent(formatIsoDateOnly(start))
            + '&endDate=' + encodeURIComponent(formatIsoDateOnly(end));
        ...
        const latestEffr = rows
            .filter((row) => String(row?.type || '').toUpperCase() === 'EFFR')
            .filter((row) => Number.isFinite(toFiniteNumber(row?.percentRate)))
            .sort((a, b) => new Date(String(b.effectiveDate || '') + 'T00:00:00Z').getTime() - new Date(String(a.effectiveDate || '') + 'T00:00:00Z').getTime())[0];

        if (!latestEffr) return null;
        const effectiveRate = toFiniteNumber(latestEffr.percentRate);
        let targetLower = toFiniteNumber(latestEffr.targetRateFrom);
        let targetUpper = toFiniteNumber(latestEffr.targetRateTo);
        ...
        return normalizeFedData({
            ...
            obsDate: {
                dff: latestEffr.effectiveDate || null,
                upper: latestEffr.effectiveDate || null,
                lower: latestEffr.effectiveDate || null
            },
            lastUpdate: Date.now()
        });
    }
    ...
    async function fetchFedRateFromAPI() {
        ...
        try {
            ...
            const nyFedRate = await fetchFedRateFromNyFed().catch((e) => {
                macroLog('NY Fed API falhou: ' + (e?.message || e), 'warn');
                return null;
            });
            if (hasUsableFedRateData(nyFedRate)) {
                macroLog('NY Fed EFFR: ' + nyFedRate.effectiveRate + '% | Target ' + nyFedRate.currentRate.lower + '%-' + nyFedRate.currentRate.upper + '%', 'success');
                return rememberFedRateData(nyFedRate);
            }
            ...
            return rememberFedRateData(freshFedData);
    ...
    async function _fetchPolymarketEventsByTag(tagSlug) {
        ...
        try {
            ...
            return _extractPolymarketEventsPayload(data);
        } catch (_) {
            return [];
        }
    }
    ...
    async function fetchFedProbabilitiesFromWorker(nextMeeting) {
        const workerUrls = CALENDAR_WORKER_URLS.length ? CALENDAR_WORKER_URLS : (CALENDAR_WORKER_URL ? [CALENDAR_WORKER_URL] : []);
        if (!workerUrls.length) return null;
        const meetingKey = getFedProbabilityMeetingKey(nextMeeting);
        for (const workerUrl of workerUrls) {
            const url = `${workerUrl}/market/fedwatch${meetingKey ? '?meetingDate=' + encodeURIComponent(meetingKey) : ''}`;
            try {
                const data = await nativeHttpGet(url, {
                    connectTimeout: 5600,
                    readTimeout: 5600,
                ...
                try { localStorage.setItem('vc_active_worker_url_v1', workerUrl); } catch (_) {}
                return {
                    ...data,
                    ...normalized,
                    source: data.source || 'Polymarket (Worker)',
                    dataSource: data.dataSource || data.rateSource || 'FRED (Worker)',
                    stale: !!data.stale
                };
            } catch (e) {
    ...
    async function fetchFedRateFromWorker() {
        const workerUrls = CALENDAR_WORKER_URLS.length ? CALENDAR_WORKER_URLS : (CALENDAR_WORKER_URL ? [CALENDAR_WORKER_URL] : []);
        if (!workerUrls.length) return null;
        for (const workerUrl of workerUrls) {
            const url = `${workerUrl}/market/fed-rate`;
            try {
                const data = await nativeHttpGet(url, {
                    connectTimeout: 5200,
                    readTimeout: 5200,
                ...
                if (!data || data.success === false) continue;
                const normalized = normalizeFedData(data);
                if (!hasUsableFedRateData(normalized)) continue;
                try { localStorage.setItem('vc_active_worker_url_v1', workerUrl); } catch (_) {}
                return {
                    ...normalized,
                    dataSource: data.dataSource || data.rateSource || normalized.dataSource || 'FRED (Worker)',
                    stale: !!data.stale
                };
            } catch (e) {
                macroLog('Worker Fed Rate indisponivel: ' + (e?.message || e), 'warn');
            }
        }
        return null;
    }
    ...
    async function fetchFedProbabilitiesDirectFromPolymarket(nextMeeting) {
        ...
        // Multi-meeting combos ("Fed decisions (Sep–Dec)") also mention pause/cut and must be ignored.
        ...
        const selectFedCandidates = (eventList) => eventList.filter((event) => {
            ...
            const marketText = (Array.isArray(event?.markets) ? event.markets : [])
                .map((market) => String(market?.question || '')).join(' ');
            ...
            return nearMeeting || _isQuestionForUpcomingMeeting(`${title} ${marketText}`, nextMeeting);
        });

        // Gamma caps /events at 100 rows per page, so a generic scan no longer reaches
        // the Fed events. Query the Fed tags directly and keep the scan as a last resort.
        const events = [];
        for (const tag of ['fed-rates', 'fomc']) {
            const tagged = await _fetchPolymarketEventsByTag(tag);
            events.push(...tagged);
        ...
        if (!fedCandidates.length) {
            ...
            for (let page = 0; page < maxPages; page++) {
                const pageEvents = await _fetchPolymarketEventsPage(pageSize, page * pageSize);
    ...
    async function updateFedWatch() {
        ...
        try {
            ...
            for (const meeting of fomcMeetings) {
                ...
                // Keep the meeting as "next" until the 14:00 ET decision is published.
                const decisionAt = new Date(meeting.date + 'T19:00:00Z');
                if (decisionAt >= today) {
        ...
        const meetingDateFormatted = nextMeeting.date ? nextMeeting.date.toLocaleDateString(window.VisorI18n?.getLocale?.() || 'en-US', { day: '2-digit', month: '2-digit', year: 'numeric' }) : '';
        ...
        if (!hasUsableFedRateData(fedData)) {
            renderFedRateHeader(null, { loading: true });
            const lastDecisionEl = document.getElementById('last-fed-decision');
        ...
        container.innerHTML = `
            ...
            <div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid rgba(255,255,255,0.1); font-size: 10px; color: #555; display: flex; flex-direction: column; gap: 4px;">
                ...
                <div style="font-size: 9px; color: #444;">
                    Probabilidades: ${probabilitySource}${probs.eventTitle ? ' · ' + probs.eventTitle : ''}${probs.marketsUsed ? ' (' + probs.marketsUsed + ' mercados)' : ''}<br>
                    DFF: ${fedData.effectiveRate ? fedData.effectiveRate.toFixed(2) + '%' : 'Atualizando'}${fedData.obsDate?.dff ? ' (' + new Date(fedData.obsDate.dff + 'T00:00:00').toLocaleDateString(window.VisorI18n?.getLocale?.() || 'en-US', {day:'2-digit',month:'2-digit'}) + ')' : ''} | CPI: ${fedData.cpi ? fedData.cpi.toFixed(1) + '%' : 'Atualizando'}${fedData.obsDate?.cpi ? ' (' + new Date(fedData.obsDate.cpi + 'T00:00:00').toLocaleDateString(window.VisorI18n?.getLocale?.() || 'en-US', {day:'2-digit',month:'2-digit'}) + ')' : ''} | Desemp: ${fedData.unemployment ? fedData.unemployment.toFixed(1) + '%' : 'Atualizando'}${fedData.obsDate?.unrate ? ' (' + new Date(fedData.obsDate.unrate + 'T00:00:00').toLocaleDateString(window.VisorI18n?.getLocale?.() || 'en-US', {day:'2-digit',month:'2-digit'}) + ')' : ''}
                </div>
                ${probs.stale ? `
                <div style="padding: 6px 8px; background: rgba(245,158,11,0.12); border-radius: 6px; font-size: 9px; color: #d6a540; line-height: 1.4;">
                    <i class="fas fa-history" style="margin-right: 3px;"></i>
        ...
        `;
            ...
            renderFedRateHeader(getUsableFedRateCache(), { stale: true, loading: true });
            container.innerHTML = `
                <div style="text-align: center; padding: 30px; color: #ef4444;">
                    <i class="fas fa-exclamation-triangle" style="font-size: 32px; margin-bottom: 12px; opacity: 0.7;"></i>
                    <p style="margin: 0; font-weight: 600;">Erro ao carregar dados</p>
    ...
    // Override robusto: mantém a UI com dados reais/cache e tenta novamente sem erro inicial.
    async function updateFedWatch(options = {}) {
        const container = document.getElementById('fed-probabilities');
        const nextMeetingEl = document.getElementById('next-fomc-meeting');
        const currentRateEl = document.getElementById('current-fed-rate');
        ...
        const bootCachedRate = getUsableFedRateCache();
        renderFedRateHeader(bootCachedRate, { stale: !!bootCachedRate, loading: !bootCachedRate });

        const hasRenderedData = !!container.querySelector('.fed-prob-item');
        if (!hasRenderedData) {
            renderFedWatchSearching(container, null, { message: 'Preparando taxa Fed e probabilidades...' });
        }
        ...
        try {
            ...
            try {
                ...
                macroLog('Não foi possível buscar próxima reunião FOMC', 'warn');
            }

            const meetingDateFormatted = nextMeeting.date ? nextMeeting.date.toLocaleDateString(window.VisorI18n?.getLocale?.() || 'en-US', { day: '2-digit', month: '2-digit', year: 'numeric' }) : '';
            if (nextMeetingEl) nextMeetingEl.innerHTML = `Reunião: <strong>${meetingDateFormatted}</strong> (${nextMeeting.daysUntil} dias)`;

            const cachedRate = getUsableFedRateCache();
            const cachedProbs = loadFedProbabilityCache(nextMeeting);
            if (cachedRate) {
                renderFedRateHeader(cachedRate, { stale: true });
            }
            if (cachedProbs && !hasRenderedData) {
                renderFedWatchProbabilities(container, cachedRate, cachedProbs);
            }

            const [workerRateResult, workerResult, directResult] = await Promise.allSettled([
                withSoftTimeout(fetchFedRateFromWorker(), 7000, null),
                withSoftTimeout(fetchFedProbabilitiesFromWorker(nextMeeting), 7000, null),
                withSoftTimeout(fetchFedProbabilitiesDirectFromPolymarket(nextMeeting), 12500, null)
            ]);

            if (workerRateResult.status === 'rejected') {
                macroLog('Erro ao buscar taxa Fed no worker: ' + (workerRateResult.reason?.message || workerRateResult.reason || 'desconhecido'), 'warn');
            }
            if (workerResult.status === 'rejected') {
                macroLog('Erro ao buscar probabilidades no worker: ' + (workerResult.reason?.message || workerResult.reason || 'desconhecido'), 'warn');
            }
            if (directResult.status === 'rejected') {
                macroLog('Erro ao buscar probabilidades direto no Polymarket: ' + (directResult.reason?.message || directResult.reason || 'desconhecido'), 'warn');
            }

            const workerRateData = normalizeFedData(workerRateResult.status === 'fulfilled' ? workerRateResult.value : null);
            const workerFedData = normalizeFedData(workerResult.status === 'fulfilled' ? workerResult.value : null);
            let fallbackFedData = null;
            if (!hasUsableFedRateData(workerRateData) && !hasUsableFedRateData(workerFedData)) {
                fallbackFedData = normalizeFedData(await withSoftTimeout(fetchFedRateFromAPI(), 9000, null).catch(() => null));
            }
            const fedData = rememberFedRateData(workerRateData)
                || rememberFedRateData(workerFedData)
                || rememberFedRateData(fallbackFedData)
                || cachedRate
                || getUsableFedRateCache();

            const workerProbs = normalizeFedProbabilityPayload(
                workerResult.status === 'fulfilled' ? workerResult.value : null,
                nextMeeting,
                'Polymarket (Worker)'
            );
            const directProbs = normalizeFedProbabilityPayload(
            ...
            let probs = null;
            if (workerProbs && !workerProbs.stale) probs = workerProbs;
            else if (directProbs && !directProbs.stale) probs = directProbs;
            else if (workerProbs) probs = workerProbs;
            else if (directProbs) probs = directProbs;
            else if (cachedProbs) {
                probs = cachedProbs;
                macroLog('Usando cache local de probabilidades Fed Watch', 'warn');
            }

            renderFedRateHeader(fedData, { stale: !!(fedData && !workerRateData && !workerFedData && !fallbackFedData), loading: !fedData });

            if (!probs) {
                renderFedWatchUnavailable(container, fedData, {
                    keepExisting: true,
                    message: 'Polymarket ainda não respondeu. Vou tentar novamente automaticamente.'
                });
                scheduleFedWatchRetry('probabilidades reais indisponíveis');
                return;
            }
            ...
            if (!probs.stale) {
                ...
                scheduleFedWatchRetry('atualizar probabilidades reais');
            }
            renderFedWatchProbabilities(container, fedData, probs);
        } catch(e) {
            macroLog('Erro updateFedWatch: ' + (e?.message || e || 'desconhecido'), 'warn');
            const fallbackRate = getUsableFedRateCache();
            renderFedRateHeader(fallbackRate, { stale: !!fallbackRate, loading: !fallbackRate });
            renderFedWatchUnavailable(container, fallbackRate, {
                keepExisting: true,
                message: 'Falha temporária de rede. Vou tentar novamente automaticamente.'
            });
            scheduleFedWatchRetry('falha temporária');
        } finally {
            window.__vcFedWatchUpdating = false;
        }
    }
    ...
    function _formatOfficialSeriesValue(config, actual, previous = null) {
        ...
        if (config.format === 'changeK') {
            ...
            return `${change >= 0 ? '+' : ''}${Math.round(change).toLocaleString(window.VisorI18n?.getLocale?.() || 'en-US')}K`;
        }

        if (config.format === 'countK') {
            return `${Math.round(actual / 1000).toLocaleString(window.VisorI18n?.getLocale?.() || 'en-US')}K`;
        }

        if (config.format === 'thousandToM') {
            return `${(actual / 1000).toLocaleString(window.VisorI18n?.getLocale?.() || 'en-US', {
        ...
        return actual.toLocaleString(window.VisorI18n?.getLocale?.() || 'en-US', {
            minimumFractionDigits: decimals,
            maximumFractionDigits: decimals
        }) + (config.suffix || '');
    }
            ...
            if (data.observations && data.observations.length > 0) {
                ...
                const history = validObs.slice(0, 6).map((item, index) => {
                    ...
                    if (fredSeries === 'PAYEMS') {
                        ...
                        return {
                            date: item.date,
                            actual: `${change >= 0 ? '+' : ''}${Math.round(change).toLocaleString(window.VisorI18n?.getLocale?.() || 'en-US')}K`,
                            previous: prev ? `${Math.round(previous).toLocaleString(window.VisorI18n?.getLocale?.() || 'en-US')}K (total)` : '-',
                            forecast: '-',
                            impact: 'high'
                        };
                    }
                    ...
                    const formattedActual = (isPercentage || needsYoY || needsMoM)
                        ...
                        : actual.toLocaleString(window.VisorI18n?.getLocale?.() || 'en-US', {
                            minimumFractionDigits: decimals,
                            maximumFractionDigits: decimals
                        });
                    ...
                    const formattedPrevious = prev
                        ? ((isPercentage || needsYoY || needsMoM)
                            ...
                            : previous.toLocaleString(window.VisorI18n?.getLocale?.() || 'en-US', {
                                minimumFractionDigits: decimals,
                                maximumFractionDigits: decimals
                            }))
                        : '-';
    ...
    async function fetchFREDReleaseDates() {
        ...
        const result = {};
        macroLog('FRED release dates: usando agenda local/cache sem proxy generico', 'info');
        return result;
        ...
        for (const [catKey, schedule] of Object.entries(OFFICIAL_SCHEDULE)) {
            ...
            for (const dateEntry of dates) {
                ...
                if (d >= from && d <= to) {
                    ...
                    events.push({
                        day: d.getDate(),
                        month: d.toLocaleDateString(window.VisorI18n?.getLocale?.() || 'en-US', { month: 'short' }).toUpperCase(),
                        time: eventTime,
    ...
    function _parseInvestingCalendarHtml(html) {
        ...
        rows.forEach((row) => {
            ...
            events.push({
                ...
                month: fullDateObj.toLocaleDateString(window.VisorI18n?.getLocale?.() || 'en-US', { month: 'short' }).toUpperCase(),
        ...
        try {
            ...
            const mappedEvents = usEvents.map(e => {
                ...
                return {
                    day: fullDateObj.getDate(),
                    month: fullDateObj.toLocaleDateString(window.VisorI18n?.getLocale?.() || 'en-US', { month: 'short' }).toUpperCase(),
                    time: new Date(e.date).toLocaleTimeString(window.VisorI18n?.getLocale?.() || 'en-US', { hour: '2-digit', minute: '2-digit', hour12: false }),
                    title: translatedTitle,
                    fullDate: fullDate,
                    country: 'EUA',
    ...
    function _renderCalendarEvents(container, events) {
        ...
        container.innerHTML = viewEvents.map((e, idx) => {
            ...
            const titleHtml = _escapeHtml(e.title || 'Evento Econômico');
            const titleAttr = _escapeAttr(e.title || '');
            const countryHtml = _escapeHtml(e.country || 'EUA');
            const timeHtml = _escapeHtml(e.time || '--:--');
            return `
            <div class="calendar-event" data-event-idx="${idx}" data-event-title="${titleAttr}" style="cursor: pointer; transition: background 0.2s; border-left: 2px solid ${impactMeta.color}99;" onclick="window.MacroAPI.showEventDetailsByIndex(${idx})">
                ...
                <div class="calendar-info">
                    <div class="calendar-title-row">
                        <div class="calendar-title">${titleHtml}</div>
                        <div class="calendar-impact-slot">${impactBadge}</div>
                    </div>
                    <div class="calendar-country">${countryHtml} • ${timeHtml}</div>
                </div>
                <i class="fas fa-chevron-right" style="color: var(--text-muted); font-size: 12px; margin-left: 8px;"></i>
            </div>
        ...
        updateInfo.textContent = `Atualizado: ${new Date().toLocaleTimeString(window.VisorI18n?.getLocale?.() || 'en-US', { hour: '2-digit', minute: '2-digit' })} • ${officialCount} datas oficiais`;
    ...
    async function showEventDetails(eventTitle, eventDate, eventData = null) {
        window.VisorMonetization?.recordAction('macro_event_open').catch(() => {});
        ...
        const dateLabel = hasValidDate
            ? eventDateTime.toLocaleDateString(window.VisorI18n?.getLocale?.() || 'en-US', { day: '2-digit', month: 'long', year: 'numeric' })
            : 'Data não informada';
        const timeLabel = hasValidDate
            ? eventDateTime.toLocaleTimeString(window.VisorI18n?.getLocale?.() || 'en-US', { hour: '2-digit', minute: '2-digit' })
            : timeRaw;
        const lastCalendarSync = calendarCache?.lastUpdate
            ? new Date(calendarCache.lastUpdate).toLocaleTimeString(window.VisorI18n?.getLocale?.() || 'en-US', { hour: '2-digit', minute: '2-digit' })
            : 'indisponível';
        const titleHtml = _escapeHtml(safeTitle);
        const dateLabelHtml = _escapeHtml(dateLabel);
        const timeLabelHtml = _escapeHtml(timeLabel);
        const sourceHtml = _escapeHtml(sourceRaw);
        const countryHtml = _escapeHtml(countryRaw);
        const lastCalendarSyncHtml = _escapeHtml(lastCalendarSync);
        const timingLabelHtml = _escapeHtml(timing.label);
        const timingRemainingHtml = _escapeHtml(timing.remaining);
        const frequencyHtml = _escapeHtml(frequency);
        const descriptionHtml = _escapeHtml(eventInfo.description);
        const expectationHtml = _escapeHtml(eventInfo.expectation || '');
            ...
            <div id="event-modal-sheet" style="background: var(--bg-secondary, #1a1a2e); width: calc(100% - 16px); max-width: 420px; border-radius: 20px; overflow: hidden; max-height: 85vh; display: flex; flex-direction: column; transform: translateY(100%); transition: transform 0.3s cubic-bezier(0.22, 1, 0.36, 1); will-change: transform; margin-bottom: calc(env(safe-area-inset-bottom, 0px) + 12px);">
                ...
                <div style="padding: 20px; border-bottom: 1px solid rgba(255,255,255,0.1); display: flex; justify-content: space-between; align-items: center; flex-shrink: 0;">
                    <div>
                        ...
                        <p style="margin: 4px 0 0; font-size: 12px; color: #888;">Data: ${dateLabelHtml} • ${timeLabelHtml}</p>
                    </div>
                ...
                <div style="padding: 16px; padding-bottom: calc(env(safe-area-inset-bottom, 0px) + 24px); overflow-y: auto; flex: 1; -webkit-overflow-scrolling: touch;">
                    ...
                    <div style="background: linear-gradient(135deg, rgba(30,41,59,0.85) 0%, rgba(15,23,42,0.95) 100%); border-radius: 14px; border: 1px solid rgba(148,163,184,0.2); padding: 14px; margin-bottom: 12px;">
                        <div style="display:flex; align-items:center; justify-content:space-between; gap:10px;">
                            <div>
                                <div style="font-size: 10px; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.4px; font-weight: 700;">${timingLabelHtml}</div>
                                <div style="font-size: 26px; font-weight: 900; color: ${timing.color}; margin-top: 4px; line-height: 1.1;">${timingRemainingHtml}</div>
                            </div>
                    ...
                    <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 14px;">
                        <div style="background: rgba(148,163,184,0.08); border-radius: 10px; padding: 10px; border: 1px solid rgba(148,163,184,0.14);">
                            ...
                            <div style="font-size: 13px; font-weight: 800; color: var(--text-primary); margin-top: 4px;">${frequencyHtml}</div>
                        </div>
                        <div style="background: rgba(148,163,184,0.08); border-radius: 10px; padding: 10px; border: 1px solid rgba(148,163,184,0.14);">
                            <div style="font-size: 9px; color: var(--text-muted); text-transform: uppercase; font-weight: 700;">Horário</div>
                            <div style="font-size: 14px; font-weight: 800; color: var(--text-primary); margin-top: 4px;">${timeLabelHtml}</div>
                        </div>
                        <div style="grid-column: 1 / -1; background: rgba(148,163,184,0.08); border-radius: 10px; padding: 10px; border: 1px solid rgba(148,163,184,0.14);">
                            ...
                            <div style="font-size: 12px; font-weight: 800; color: var(--text-primary); margin-top: 4px;">${sourceHtml}</div>
                        </div>
                    </div>
                    ...
                    <div style="margin-bottom: 10px; padding: 12px; background: rgba(59,130,246,0.08); border-radius: 10px; border: 1px solid rgba(59,130,246,0.22);">
                        ...
                        <div style="font-size: 12px; color: #dbeafe; line-height: 1.5;">${descriptionHtml}</div>
                        <div style="margin-top: 8px; font-size: 11px; color: #bfdbfe;">País: ${countryHtml} • Sync calendário: ${lastCalendarSyncHtml}</div>
                    </div>
                    ...
                    ${eventInfo.expectation ? `
                        <div style="margin-top: 16px; padding: 12px; background: rgba(59, 130, 246, 0.1); border-radius: 10px; border-left: 3px solid #3b82f6;">
                            ...
                            <p style="margin: 0; color: #aaa; font-size: 12px; line-height: 1.5;">${expectationHtml}</p>
                        </div>
                    ` : ''}
                </div>
            </div>
