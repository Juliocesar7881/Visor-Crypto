                    ...
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
                        <div style="display:flex;align-items:center;gap:10px;">
                            <div style="width:36px;height:36px;background:rgba(245,158,11,0.12);border:1px solid rgba(245,158,11,0.35);border-radius:10px;display:inline-flex;align-items:center;justify-content:center;">
                                <span style="font-size:18px;">[ALERTA]</span>
                            </div>
...
let _taRawDataCache = {};
        ...
        function generateTechnicalAnalysis(data, symbol) {
            ...
            let baseConfidence = Math.round(Math.min(alignmentRatio * 108 + Math.abs(totalScore) * 3.1, 100));
            ...
            const calcConsensusMultiplier = (details) => {
                ...
                if (dominant >= 5) return 1.48; // todos concordam
                if (dominant >= 4) return 1.32; // maioria forte
                if (dominant >= 3) return 1.15; // consenso moderado
                return 1.00;
            };
            ...
            if (marketRegime.isTrending) {
                ...
                if ((totalScore > 0 && trendDirection > 0) || (totalScore < 0 && trendDirection < 0)) {
                    confidence = Math.min(confidence + 12, 100); // Aligned with trend
                } else {
                    confidence = Math.max(confidence - 7, 10); // Counter-trend
                }
            }
        ...
        function detectTrendStart(klines1m, klines5m, klines15m, klines1h, currentPrice) {
            ...
            // 5. PADRÍO ENGULFING (Candlestick de reversão)
        ...
        function buildLiquidationRiskMap(realLiquidations, currentPrice) {
            ...
            if (pendingLongUSD <= 0 && pendingShortUSD <= 0) {
                const oiUsd = parseFloat(realLiquidations.openInterestUSD || 0);
                const longPctFallback = parseFloat(realLiquidations.longPct || 0);
                const shortPctFallback = parseFloat(realLiquidations.shortPct || 0);
                if (oiUsd > 0 && (longPctFallback > 0 || shortPctFallback > 0)) {
                    const pctSum = Math.max(1, longPctFallback + shortPctFallback);
                    pendingLongUSD = oiUsd * (longPctFallback / pctSum);
                    pendingShortUSD = oiUsd * (shortPctFallback / pctSum);
                }
            }
        ...
        function generateLocalSummary(signalType, confidence, indicators, symbol) {
            ...
            if (indicators.bigTechMacro) {
                ...
                if (bt.treasuryYields?.isInverted) {
                    bigTechAnalysis += '\n[ALERTA] CURVA DE JUROS INVERTIDA — sinal de recessão que pressiona risk assets';
                }
            }
...
O CVD mostra ${indicators.cvdSignal === 'bullish' ? 'predomin de compras agressivas a mercado' : indicators.cvdSignal === 'absorption_bullish' ? 'ABSORÇÍO de vendas por compradores (baleias acumulando)' : 'fluxo equilibrado'}.
...
${indicators.rsiSignal === 'oversold' ? '[ALERTA] RSI em sobrevenda - momento ótimo para entrada!' : ''}
...
O CVD mostra ${indicators.cvdSignal === 'bearish' ? 'predomin de vendas agressivas' : indicators.cvdSignal === 'absorption_bearish' ? 'ABSORÇÍO de compras por vendedores (distribuição institucional)' : 'pressão vendedora crescente'}.
...
${indicators.rsiSignal === 'overbought' ? '[ALERTA] RSI em sobrecompra - momento ótimo para short!' : ''}
...
Confiança: ${confidence}%`;
                ...
                if (currentPrice > indicators.vah) {
                    ...
                    recommendationMsg = ` Recomendação: NÍO OPERAR agora.
...
[ALERTA] Confiança para operação: BAIXA. Sem sinal direcional.`;
            }
        }
...
- A confiança DEVE ser EXATAMENTE ${dataPayload.confidence}% — este valor já foi calculado pelo motor de confluência, NÍO calcule nem invente outro valor
...
- NÍO invente dados, use APENAS os fornecidos`;
        ...
        async function updateAISummaryInModal(analysis, symbol) {
            ...
            if (aiResult) {
                ...
                const prefix = noKey
                    ? ' Relatório em modo local. Para IA remota via Groq, configure CALENDAR_WORKER_URL e GROQ_API_KEY no Worker.\n\n'
                    : '[ALERTA] IA indisponível — análise local:\n\n';
        ...
        function renderLiquidationMapSection(realLiquidations, liquidationRiskMap, formatBigNumber, formatPrice) {
            ...
            const modeLabel = liquidationRiskMap.mode === 'pending-levels'
                ? 'Níveis reais por dist do preço'
                : 'Distribuição agregada por Open Interest';

            const heatmapRows = (liquidationRiskMap.bucketData || []).map((bucket) => {
                const maxBucket = Math.max(1, liquidationRiskMap.maxBucketUSD || 1);
                const shortIntensity = Math.min(1, (bucket.shortUSD || 0) / maxBucket);
                const longIntensity = Math.min(1, (bucket.longUSD || 0) / maxBucket);
                const shortAlpha = (0.12 + shortIntensity * 0.65).toFixed(3);
                const longAlpha = (0.12 + longIntensity * 0.65).toFixed(3);

                return `
                <div style="display: grid; grid-template-columns: 78px 1fr 1fr; gap: 6px; align-items: stretch; margin-bottom: 6px;">
                    <div style="font-size: 10px; color: var(--text-muted); font-weight: 700; display:flex; align-items:center;">${bucket.label}</div>
                    <div style="border-radius: 8px; padding: 6px; border: 1px solid rgba(239, 68, 68, 0.28); background: rgba(239, 68, 68, ${shortAlpha});">
                        <div style="font-size: 9px; color: #fee2e2; font-weight: 700; letter-spacing: 0.2px;">${shortIntensity >= 0.72 ? 'ALTO' : shortIntensity >= 0.42 ? 'MÉDIO' : 'BAIXO'}</div>
                        <div style="font-size: 9px; color: #fca5a5; margin-top: 2px; white-space: nowrap;">${formatBigNumber(bucket.shortUSD || 0)}</div>
                    </div>
                    <div style="border-radius: 8px; padding: 6px; border: 1px solid rgba(34, 197, 94, 0.28); background: rgba(34, 197, 94, ${longAlpha});">
                        <div style="font-size: 9px; color: #dcfce7; font-weight: 700; letter-spacing: 0.2px;">${longIntensity >= 0.72 ? 'ALTO' : longIntensity >= 0.42 ? 'MÉDIO' : 'BAIXO'}</div>
                        <div style="font-size: 9px; color: #86efac; margin-top: 2px; white-space: nowrap;">${formatBigNumber(bucket.longUSD || 0)}</div>
                    </div>
                </div>
                `;
            }).join('');
                ...
                <div class="ta-section">
                    ...
                    <div style="margin-top: 12px; padding: 12px; border-radius: 10px; background: var(--bg-tertiary); border: 1px solid rgba(148, 163, 184, 0.18);">
                        <div style="font-size: 11px; color: var(--text-muted); font-weight: 700; margin-bottom: 10px; text-transform: uppercase;">
                            Heatmap de risco por distância (% do preço atual: $${formatPrice(realLiquidations.currentPrice || 0)})
                        </div>
                        <div style="display: grid; grid-template-columns: 78px 1fr 1fr; gap: 6px; margin-bottom: 6px; align-items: center;">
                            <div style="font-size: 9px; color: var(--text-muted); font-weight: 700; text-transform: uppercase;">Faixa</div>
                            <div style="font-size: 9px; color: #f87171; font-weight: 700; text-transform: uppercase;">Shorts em risco</div>
                            <div style="font-size: 9px; color: #4ade80; font-weight: 700; text-transform: uppercase;">Longs em risco</div>
                        </div>
                        ${heatmapRows}
                        <div style="margin-top: 8px; font-size: 9px; color: var(--text-muted); display:flex; justify-content:space-between;">
                            <span>Cor fraca = baixa concentração</span>
                            <span>Cor forte = alta concentração</span>
                        </div>
                    </div>
            ...
            body.innerHTML = `
                ...
                <div class="ta-signal-card ${signalType}">
                    <div class="ta-signal-header">
                        ...
                        <div class="ta-signal-info">
                            ...
                            ${isV4Active ? `<div style="display: flex; align-items: center; gap: 8px; margin-top: 4px; flex-wrap: wrap;">
                                ...
                                ${analysis.dataIntegrity && !analysis.dataIntegrity.valid ? `<span style="font-size: 9px; padding: 2px 6px; border-radius: 4px; background: rgba(239,68,68,0.2); color: #f87171; font-weight: 700;">[ALERTA] DADOS</span>` : ''}
                            </div>
                    ...
                    <div style="padding: 20px; background: var(--bg-tertiary); border-radius: 12px; text-align: center;">
                        <div style="font-size: 16px; font-weight: 700; color: #f59e0b; margin-bottom: 8px;">[ALERTA] NÍO RECOMENDADO OPERAR</div>
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                            <div>
                                <div style="font-size: 16px; font-weight: 800; color: ${analysis.btcAlignment.alignment === 'ALIGNED' ? '#22c55e' : analysis.btcAlignment.alignment === 'DIVERGING' ? '#ef4444' : '#94a3b8'};">
                                    ${analysis.btcAlignment.alignment === 'ALIGNED' ? 'OK ALINHADO' : analysis.btcAlignment.alignment === 'DIVERGING' ? '[ALERTA] DIVERGENTE' : '⚖️ NEUTRO'}
                                </div>
                    ...
                    <div style="padding: 14px; background: ${analysis.setupStats.quality === 'EXCELENTE' ? 'rgba(34,197,94,0.06)' : analysis.setupStats.quality === 'BOM' ? 'rgba(59,130,246,0.06)' : analysis.setupStats.quality === 'MÉDIO' ? 'rgba(245,158,11,0.06)' : 'rgba(239,68,68,0.06)'}; border-radius: 12px; border: 1px solid ${analysis.setupStats.quality === 'EXCELENTE' ? 'rgba(34,197,94,0.2)' : analysis.setupStats.quality === 'BOM' ? 'rgba(59,130,246,0.2)' : analysis.setupStats.quality === 'MÉDIO' ? 'rgba(245,158,11,0.2)' : 'rgba(239,68,68,0.2)'};">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                            <span style="font-size: 14px; font-weight: 800; color: ${analysis.setupStats.quality === 'EXCELENTE' ? '#22c55e' : analysis.setupStats.quality === 'BOM' ? '#3b82f6' : analysis.setupStats.quality === 'MÉDIO' ? '#f59e0b' : '#ef4444'};">
                                ${analysis.setupStats.quality === 'EXCELENTE' ? '' : analysis.setupStats.quality === 'BOM' ? '' : analysis.setupStats.quality === 'MÉDIO' ? '' : '[ALERTA]'} ${analysis.setupStats.quality}
                            </span>
                        ...
                        <div style="margin-top: 8px; padding: 6px 10px; background: rgba(245,158,11,0.1); border-radius: 6px; font-size: 10px; color: #f59e0b; text-align: center;">
                            [ALERTA] Amostra pequena — dados estatisticamente insuficientes
                        </div>
                        ` : ''}
                    </div>
                </div>
                ...
                <div class="ta-section">
                    ...
                    <div style="padding: 12px; background: var(--bg-tertiary); border-radius: 12px; overflow: hidden;">
                        ...
                        <div style="padding: 8px 12px; background: rgba(239,68,68,0.12); border: 1px solid rgba(239,68,68,0.25); border-radius: 8px; margin-bottom: 16px;">
                            <div style="font-size: 11px; font-weight: 700; color: #ef4444;">[ALERTA] Curva de Juros INVERTIDA (3M-10Y: ${bigTechMacro.treasuryYields.yieldCurveSpread?.toFixed(2) || '?'}%) — Sinal de recessão</div>
                        </div>
                        ` : `
                    ...
                    <div style="background: var(--bg-tertiary); border-radius: 12px; padding: 16px; margin-bottom: 16px;">
                        ...
                        <div style="margin-top: 12px; padding: 10px; background: ${parseFloat(indicators.orderFlow.fundingRate) > 0.03 ? 'rgba(239, 68, 68, 0.15)' : parseFloat(indicators.orderFlow.fundingRate) < -0.01 ? 'rgba(34, 197, 94, 0.15)' : 'rgba(245, 158, 11, 0.15)'}; border-radius: 8px;">
                            <div style="font-size: 11px; font-weight: 600; color: ${parseFloat(indicators.orderFlow.fundingRate) > 0.03 ? '#ef4444' : parseFloat(indicators.orderFlow.fundingRate) < -0.01 ? '#22c55e' : '#f59e0b'};">
                                ${parseFloat(indicators.orderFlow.fundingRate) > 0.05 ? '[ALERTA] MUITO ALTO - Mercado sobrecomprado, pressão de venda iminente' : parseFloat(indicators.orderFlow.fundingRate) > 0.03 ? '▲ ALTO - Longs dominantes, risco de liquidação cascata' : parseFloat(indicators.orderFlow.fundingRate) > 0.01 ? '📊 LEVEMENTE ALTO - Longs pagando shorts' : parseFloat(indicators.orderFlow.fundingRate) < -0.03 ? '[ALERTA] MUITO NEGATIVO - Shorts dominantes, possível squeeze' : parseFloat(indicators.orderFlow.fundingRate) < -0.01 ? '▼ NEGATIVO - Shorts pagando longs' : '⚖️ NEUTRO - Mercado equilibrado'}
                            </div>
                        </div>
                    </div>
                ...
                <div class="ta-section">
                    ...
                    <div class="ta-indicator-grid">
                        <div class="ta-indicator-item">
                            ...
                            <div class="ta-indicator-change">${parseFloat(indicators.sentiment.rsi) < 30 ? ' Oportunidade' : parseFloat(indicators.sentiment.rsi) > 70 ? '[ALERTA] Cuidado' : 'Normal'}</div>
                        </div>
                        <div class="ta-indicator-item">
                            <div class="ta-indicator-header">
                    ...
                    <div style="padding: 16px; background: ${analysis.crashState.isCrash || analysis.crashState.isRapidPump ? 'rgba(239,68,68,0.1)' : 'rgba(234,179,8,0.1)'}; border-radius: 12px; border: 1px solid ${analysis.crashState.isCrash || analysis.crashState.isRapidPump ? 'rgba(239,68,68,0.3)' : 'rgba(234,179,8,0.3)'};">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 8px;">
                            ...
                            <div style="background: var(--bg-tertiary); padding: 8px; border-radius: 8px; text-align: center;">
                                ...
                                <div style="font-size: 13px; font-weight: 700; color: ${analysis.crashState.volumeSpikeDetected ? '#ef4444' : '#94a3b8'};">${analysis.crashState.volumeSpikeDetected ? 'SIM' : 'NÍO'}</div>
                            </div>
                        </div>
                    ...
                    <div style="padding: 16px; background: rgba(14,165,233,0.08); border-radius: 12px; border: 1px solid rgba(14,165,233,0.2);">
                        ...
                        <div style="text-align: center; margin-bottom: 14px;">
                            ...
                            <div style="display: inline-block; margin-top: 8px; padding: 4px 12px; border-radius: 8px; background: ${analysis.positionSize.riskLevel === 'CONSERVADOR' ? 'rgba(34,197,94,0.2)' : analysis.positionSize.riskLevel === 'MODERADO' ? 'rgba(234,179,8,0.2)' : 'rgba(239,68,68,0.2)'}; color: ${analysis.positionSize.riskLevel === 'CONSERVADOR' ? '#22c55e' : analysis.positionSize.riskLevel === 'MODERADO' ? '#eab308' : '#ef4444'}; font-size: 10px; font-weight: 700;">
                                ${analysis.positionSize.riskLevel === 'CONSERVADOR' ? ' Risco Baixo' : analysis.positionSize.riskLevel === 'MODERADO' ? '⚖️ Risco Médio' : analysis.positionSize.riskLevel === 'AGRESSIVO' ? ' Risco Alto' : '[ALERTA] Risco Máximo'}
                            </div>
                        </div>
                        <!-- Explicação simples -->
                        <div style="display: flex; flex-direction: column; gap: 6px;">
                            ...
                            <div style="padding: 10px; background: rgba(239,68,68,0.1); border-radius: 8px; border: 1px solid rgba(239,68,68,0.2);">
                                <span style="font-size: 11px; color: #ef4444; font-weight: 600;">[ALERTA] Mercado em queda detectado — posição reduzida automaticamente</span>
                            </div>
                            ` : ''}
                        </div>
                ...
                <div class="ta-section" style="border: 1px solid ${analysis.riskEngine.killSwitchActive ? 'rgba(239,68,68,0.4)' : 'rgba(245,158,11,0.2)'}; background: ${analysis.riskEngine.killSwitchActive ? 'linear-gradient(135deg, rgba(239,68,68,0.08) 0%, transparent 100%)' : 'none'};">
                    <div class="ta-section-header">
                        ...
                        <div>
                            ...
                            <div class="ta-section-subtitle">${analysis.riskEngine.killSwitchActive ? '[ALERTA] Kill Switch ATIVO — ' + (analysis.riskEngine.killSwitchReason || 'pausa forçada') : 'Gestão din de risco'}</div>
                        </div>
                    </div>
                ...
                <!-- [ALERTA] AVISO LEGAL - Botão que abre modal -->
