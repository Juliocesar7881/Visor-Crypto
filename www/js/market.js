            ...
            try {
                ...
                if (raw) {
                    ...
                    if (c.val && c.ts && (Date.now() - c.ts) < BTC_DOM_CACHE_TTL) {
                        document.getElementById('btc-dominance').textContent = `${c.val.toFixed(2)}%`;
        ...
        async function fetchBTCStats() {
            ...
            if (cached && cached.mcap) {
                document.getElementById('market-cap-btc').textContent = formatMcap(cached.mcap);
            ...
            try {
                ...
                if (marketCap > 0) {
                    document.getElementById('market-cap-btc').textContent = formatMcap(marketCap);
        ...
        async function fetchETHStats() {
            ...
            if (cached && cached.mcap) {
                document.getElementById('market-cap').textContent = formatMcap(cached.mcap);
            ...
            try {
                ...
                if (marketCap > 0) {
                    document.getElementById('market-cap').textContent = formatMcap(marketCap);
                    const changeToCache = mcapChange !== 0 ? mcapChange : (cached?.change || 0);
        ...
        async function fetchVolume() {
            ...
            if (cachedVolBtc && cachedVolBtc.vol) {
                ...
                document.getElementById('live-volume-btc').textContent = fv > 1e9 ? `$${(fv / 1e9).toFixed(2)}B` : `$${(fv / 1e6).toFixed(2)}M`;
            ...
            if (cachedVolEth && cachedVolEth.vol) {
                ...
                document.getElementById('live-volume').textContent = fv > 1e9 ? `$${(fv / 1e9).toFixed(2)}B` : `$${(fv / 1e6).toFixed(2)}M`;
