        ...
        async function fetchGlobalData() {
            ...
            try {
                const workerUrl = getWorkerMarketUrl('/market/global');
                if (workerUrl) {
                    const response = await fetchWithTimeout(workerUrl, {}, 2500);
                    if (response.ok) {
                        const data = await response.json();
                        const workerDom = Number(data.btcDominance || 0);
                        if (data.success !== false && workerDom > 0) {
                            updateBtcDominanceUI(workerDom);
                            try {
                                localStorage.setItem(BTC_DOM_CACHE_KEY, JSON.stringify({
                                    val: workerDom,
                                    raw: data.btcDominanceRaw || null,
                                    ts: Number(data.updatedAt || Date.now()) || Date.now(),
                                    stale: !!data.stale,
