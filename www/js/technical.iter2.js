                ...
                <div style="background:linear-gradient(180deg,#1c1c35 0%,#141423 100%);border:1px solid rgba(245,158,11,0.3);border-radius:24px 24px 0 0;max-width:480px;width:100%;padding:24px 22px 36px;max-height:88vh;overflow-y:auto;box-shadow:0 -8px 40px rgba(0,0,0,0.5);animation:slideUpCard 0.3s cubic-bezier(0.22,1,0.36,1) forwards;">
                    ...
                    <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:16px;">
                        <div style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;background:rgba(245,158,11,0.07);border:1px solid rgba(245,158,11,0.18);border-radius:11px;">
                            <span style="color:#f59e0b;font-size:14px;flex-shrink:0;margin-top:1px;">ðŸ“‹</span>
                        ...
                        <div style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;background:rgba(239,68,68,0.07);border:1px solid rgba(239,68,68,0.18);border-radius:11px;">
                            <span style="color:#ef4444;font-size:14px;flex-shrink:0;margin-top:1px;">âš¡</span>
                        ...
                        <div style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;background:rgba(99,102,241,0.07);border:1px solid rgba(99,102,241,0.18);border-radius:11px;">
                            <span style="color:#a78bfa;font-size:14px;flex-shrink:0;margin-top:1px;">ðŸ¤–</span>
                        ...
                        <div style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;background:rgba(88,166,255,0.07);border:1px solid rgba(88,166,255,0.18);border-radius:11px;">
                            <span style="color:#58a6ff;font-size:14px;flex-shrink:0;margin-top:1px;">ðŸ”</span>
...
let _taRawDataCache = {};
        ...
        async function fetchTechnicalAnalysisData(symbol) {
            ...
            const [
                ...
                fetch(`https://fapi.binance.com/futures/data/takerlongshortRatio?symbol=${symbol}&period=1h&limit=24`)
                    .then(r => r.json()).catch(() => []),
                // LIQUIDAÃ‡Ã•ES FORÃ‡ADAS REAIS (últimas 100) - DADOS REAIS DA BINANCE
            ...
            const result = {
                ...
                forceOrders: forceOrders || [],  // LIQUIDAÃ‡Ã•ES REAIS
        ...
        function generateTechnicalAnalysis(data, symbol) {
            ...
            // ============================================
            // HEATMAP DE LIQUIDAÃ‡Ã•ES (estilo Coinglass)
            // ============================================
            ...
            // ============================================
            // MAPA DE RISCO DE LIQUIDAÃ‡ÃO PENDENTE (peso variável até 2.2)
            // ============================================
            ...
            // ============================================
            // S/R BREAKOUT DETECTION â†’ Confluence
            // ============================================
            ...
            if (sr && sr.supportCount >= 3 && currentPrice < sr.support * 1.01 && currentPrice > sr.support * 0.99) {
                confluenceScore += 2.5;
                confluenceDetails.push({ name: 'Suporte Histórico', value: `>= 3 toques nÃvel ${sr.support}`, signal: 'LONG', weight: 2.5, color: '#22c55e' });
            }
            if (sr && sr.resistanceCount >= 3 && currentPrice > sr.resistance * 0.99 && currentPrice < sr.resistance * 1.01) {
                confluenceScore -= 2.5;
                confluenceDetails.push({ name: 'Resistência Histórica', value: `>= 3 toques nÃvel ${sr.resistance}`, signal: 'SHORT', weight: 2.5, color: '#ef4444' });
            }
            ...
            // Pine confidence modulation (impacta só quando score >= 75)
            if (pineV7.active) {
        ...
        // ============================================
        // ANÁLISE DE PADRÃ•ES GRÁFICOS MULTI-TIMEFRAME
        // ============================================
        ...
        function analyzeTimeframe(klines, currentPrice) {
            ...
            // ===== DETECÃ‡ÃO DE PADRÃ•ES DE CANDLESTICK =====
            const patterns = [];
            ...
            if (len >= 3) {
                ...
                if (fullRange > 0 && body / fullRange < 0.1) {
                    patterns.push({ name: 'Doji', icon: 'âœš', type: 'reversal', bias: 'neutro' });
                }
                // Hammer / Inverted Hammer
                if (fullRange > 0 && lowerWick > body * 2 && upperWick < body * 0.5) {
                    const isBull = c > o;
                    patterns.push({ name: 'Martelo', icon: 'ðŸ”¨', type: 'reversal', bias: isBull ? 'alta' : 'alta' });
                }
                if (fullRange > 0 && upperWick > body * 2 && lowerWick < body * 0.5) {
                    patterns.push({ name: 'Shooting Star', icon: 'â­', type: 'reversal', bias: 'baixa' });
                }
                // Engulfing
                if (c > o && prev_c < prev_o && c > prev_o && o < prev_c) {
                    patterns.push({ name: 'Engolfo Alta', icon: 'ðŸŸ¢', type: 'reversal', bias: 'alta' });
                }
                if (c < o && prev_c > prev_o && c < prev_o && o > prev_c) {
                    patterns.push({ name: 'Engolfo Baixa', icon: 'ðŸ”´', type: 'reversal', bias: 'baixa' });
                }
                // Pin Bar
                if (fullRange > 0 && lowerWick / fullRange > 0.6 && body / fullRange < 0.2) {
                    patterns.push({ name: 'Pin Bar Alta', icon: 'ðŸ“Œ', type: 'reversal', bias: 'alta' });
                }
                if (fullRange > 0 && upperWick / fullRange > 0.6 && body / fullRange < 0.2) {
                    patterns.push({ name: 'Pin Bar Baixa', icon: 'ðŸ“Œ', type: 'reversal', bias: 'baixa' });
                }
                // Marubozu (corpo > 90% da range)
                if (fullRange > 0 && body / fullRange > 0.9) {
                    const isBull = c > o;
                    patterns.push({ name: isBull ? 'Marubozu Alta' : 'Marubozu Baixa', icon: isBull ? 'ðŸ’ª' : 'ðŸ’€', type: 'continuation', bias: isBull ? 'alta' : 'baixa' });
                }
                ...
                if (len >= 4) {
                    ...
                    if (c3[0] > o3[0] && c3[1] > o3[1] && c3[2] > o3[2] && c3[1] > c3[0] && c3[2] > c3[1]) {
                        patterns.push({ name: '3 Soldados', icon: 'ðŸŸ¢ðŸŸ¢ðŸŸ¢', type: 'continuation', bias: 'alta' });
                    }
                    if (c3[0] < o3[0] && c3[1] < o3[1] && c3[2] < o3[2] && c3[1] < c3[0] && c3[2] < c3[1]) {
                        patterns.push({ name: '3 Corvos', icon: 'ðŸ”´ðŸ”´ðŸ”´', type: 'continuation', bias: 'baixa' });
                    }
                }
                ...
                if (len >= 4) {
                    ...
                    if (pp.c < pp.o && mid.body < prevBody * 0.3 && c > o && c > (pp.o + pp.c) / 2) {
                        patterns.push({ name: 'Morning Star', icon: 'ðŸŒ…', type: 'reversal', bias: 'alta' });
                    }
                    if (pp.c > pp.o && mid.body < prevBody * 0.3 && c < o && c < (pp.o + pp.c) / 2) {
                        patterns.push({ name: 'Evening Star', icon: 'ðŸŒ†', type: 'reversal', bias: 'baixa' });
                    }
                }
            }
        ...
        function findKeyLevels(klines, currentPrice) {
            ...
            // Filtrar níveis próximos ao preço atual (Â±10%)
        ...
        // ============================================
        // DETECÃ‡ÃO DE INÃCIO DE TENDÃŠNCIA (REAL)
        ...
        function detectTrendStart(klines1m, klines5m, klines15m, klines1h, currentPrice) {
            ...
            // 4. DIVERGÃŠNCIA RSI - Preço faz novo low, RSI não confirma
        ...
        // ============================================
        // HEATMAP DE LIQUIDAÃ‡Ã•ES — DADOS REAIS
        ...
        function analyzeRealLiquidations(forceOrders, currentPrice, openInterest, longShortRatio) {
            ...
            // â”€â”€â”€ FONTE PRIMÁRIA: Ordens forçadas reais da Binance â”€â”€â”€
            ...
            if (validForceOrders.length > 0) {
                ...
                validForceOrders.forEach(order => {
                    ...
                    // side=BUY â†’ SHORT foi liquidado (compra forçada p/ fechar short)
                    // side=SELL â†’ LONG foi liquidado (venda forçada p/ fechar long)
            ...
            // â”€â”€â”€ FALLBACK: Estimativa via Open Interest â”€â”€â”€
        ...
        function buildLiquidationRiskMap(realLiquidations, currentPrice) {
            ...
            bucketDefs.forEach(def => {
                ...
                if (def.max === Number.POSITIVE_INFINITY) {
                    ...
                    def.label = _fmtP(lo) + ' â€“ ' + _fmtP(hiUp);
                }
            });
        ...
        function generateLocalSummary(signalType, confidence, indicators, symbol) {
            ...
            if (indicators.whaleActivity) {
                ...
                if (whale.level === 'muito_alta' || whale.level === 'alta') {
                    if (whale.direction === 'compra') {
                        whaleAnalysis = 'ðŸ‹ Baleias: Atividade ALTA com predominância de COMPRA - institucionais acumulando.';
                    } else if (whale.direction === 'venda') {
                        whaleAnalysis = 'ðŸ‹ Baleias: Atividade ALTA com predominância de VENDA - institucionais distribuindo.';
                    } else {
                        whaleAnalysis = 'ðŸ‹ Baleias: Atividade ALTA detectada - monitorar direção.';
                    }
                } else if (whale.level === 'moderada') {
                    whaleAnalysis = 'ðŸ‹ Baleias: Atividade moderada no mercado.';
                }
            }
            ...
            if (indicators.bookImbalance) {
                ...
                if (ratio > 1.5) {
                    bookAnalysis = 'ðŸ“— Order Book: Forte pressão compradora (${ratio.toFixed(2)}x mais bids que asks).';
                } else if (ratio < 0.67) {
                    bookAnalysis = 'ðŸ“• Order Book: Forte pressão vendedora (${(1/ratio).toFixed(2)}x mais asks que bids).';
                }
            }
            ...
            if (indicators.marketStructure) {
                ...
                if (s.structureDescription) {
                    structureAnalysis = `ðŸ“ Estrutura: ${s.structureDescription}`;
                }
                if (s.liquiditySweeps?.detected) {
                    structureAnalysis += `\nðŸŽ¯ ${s.liquiditySweeps.description}`;
                }
            }
            if (indicators.cvdAdvanced) {
                ...
                if (c.absorption) cvdAdvAnalysis += `ðŸ‹ CVD: ${c.absorption.description}\n`;
                if (c.breakout) cvdAdvAnalysis += `ðŸ’¥ CVD: ${c.breakout.description}\n`;
            }
            ...
            if (indicators.bigTechMacro) {
                ...
                bigTechAnalysis = `ðŸ¢ Big Tech: ${bt.bigTechSentiment} (${techSummary})`;
                if (bt.fearGreed) {
                    bigTechAnalysis += `\nðŸŽ­ Fear & Greed: ${bt.fearGreed.value}/100 — ${bt.fearGreed.classification}`;
                }
                if (bt.indices?.length > 0) {
                    ...
                    if (vix) bigTechAnalysis += ` | ðŸ˜± VIX: ${vix.price.toFixed(1)}`;
                }
                if (bt.treasuryYields?.isInverted) {
                    bigTechAnalysis += '\nâš ï¸ CURVA DE JUROS INVERTIDA — sinal de recessão que pressiona risk assets';
                }
            }
...
O CVD mostra ${indicators.cvdSignal === 'bullish' ? 'predominância de compras agressivas a mercado' : indicators.cvdSignal === 'absorption_bullish' ? 'ABSORÃ‡ÃO de vendas por compradores (baleias acumulando)' : 'fluxo equilibrado'}.
...
${indicators.rsiSignal === 'oversold' ? 'âš ï¸ RSI em sobrevenda - momento ótimo para entrada!' : ''}
${indicators.lsSignal === 'bullish' ? 'ðŸ’¡ Long/Short Ratio indica shorts excessivos - potencial short squeeze.' : ''}
...
O CVD mostra ${indicators.cvdSignal === 'bearish' ? 'predominância de vendas agressivas' : indicators.cvdSignal === 'absorption_bearish' ? 'ABSORÃ‡ÃO de compras por vendedores (distribuição institucional)' : 'pressão vendedora crescente'}.
...
${indicators.rsiSignal === 'overbought' ? 'âš ï¸ RSI em sobrecompra - momento ótimo para short!' : ''}
${indicators.lsSignal === 'bearish' ? 'ðŸ’¡ Long/Short Ratio indica longs excessivos - potencial long squeeze.' : ''}
...
Confiança: ${confidence}%`;
                ...
                if (currentPrice > indicators.vah) {
                    ...
                    recommendationMsg = `ðŸš« Recomendação: NÃO OPERAR agora.
â€¢ O preço JÁ rompeu VAH - aguarde reteste da VAH ($${indicators.vah?.toFixed(2)}) como SUPORTE para LONG
â€¢ Ou aguarde retorno para dentro da área de valor para SHORT
â€¢ Evite comprar no topo sem confirmação de volume`;
                    ...
                    recommendationMsg = `ðŸš« Recomendação: NÃO OPERAR agora.
â€¢ O preço JÁ rompeu VAL - aguarde reteste da VAL ($${indicators.val?.toFixed(2)}) como RESISTÃŠNCIA para SHORT
â€¢ Ou aguarde retorno para dentro da área de valor para LONG
â€¢ Evite vender no fundo sem confirmação de volume`;
                } else {
                    ...
                    recommendationMsg = `ðŸš« Recomendação: NÃO OPERAR agora.
â€¢ Aguarde rompimento claro da VAH ($${indicators.vah?.toFixed(2)}) com volume para LONG
â€¢ Aguarde rompimento claro da VAL ($${indicators.val?.toFixed(2)}) com volume para SHORT`;
                }
                
                return `â³ NEUTRO — ${crypto}
...
${v2Block ? '--- Análise Detalhada ---\n' + v2Block + '\n\n' : ''}Indicadores atuais:
â€¢ Funding Rate: ${indicators.fundingSignal}
â€¢ CVD: ${indicators.cvdSignal}
â€¢ Order Book: ${indicators.bookSignal}

âš ï¸ Confiança para operação: BAIXA. Sem sinal direcional.`;
            }
        }
...
- Comece com o sinal (📈 LONG / 📉 SHORT / â³ NEUTRO)
...
- NÃO invente dados, use APENAS os fornecidos`;
        ...
        async function updateAISummaryInModal(analysis, symbol) {
            ...
            if (aiResult) {
                ...
                const prefix = noKey
                    ? 'â„¹ï¸ Relatório em modo local. Para IA remota via Groq, configure CALENDAR_WORKER_URL e GROQ_API_KEY no Worker.\n\n'
                    : 'âš ï¸ IA indisponível — análise local:\n\n';
            ...
            return `
                <div class="ta-section">
                    ...
                    <div style="margin-top: 10px; font-size: 10px; color: var(--text-muted);">
                        Atualização: ${realLiquidations.lastUpdate || '-'} | Este mapa influencia confluência, e confiança final.
                    </div>
                </div>
            `;
        }
        ...
        function renderTechnicalAnalysis(analysis, crypto) {
            ...
            // â•â•â• SYNC CONFIDENCE WITH MARKET REGIME â•â•â•
            ...
            body.innerHTML = `
                ...
                <div class="ta-signal-card ${signalType}">
                    <div class="ta-signal-header">
                        ...
                        <div class="ta-signal-info">
                            ...
                            ${isV4Active ? `<div style="display: flex; align-items: center; gap: 8px; margin-top: 4px; flex-wrap: wrap;">
                                ...
                                <span style="font-size: 10px; color: var(--text-muted); white-space: nowrap;">âœ… ${analysis.v4GatesPassed || 0}/${analysis.v4GatesTotal || 9} confirmações</span>
                                ${analysis.dataIntegrity && !analysis.dataIntegrity.valid ? `<span style="font-size: 9px; padding: 2px 6px; border-radius: 4px; background: rgba(239,68,68,0.2); color: #f87171; font-weight: 700;">âš ï¸ DADOS</span>` : ''}
                            </div>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-top: 8px;">
                        ...
                        <div style="background: rgba(34,197,94,0.2); padding: 10px; border-radius: 8px; text-align: center; border: 1px solid rgba(34,197,94,0.4);">
                            <div style="font-size: 9px; color: #10b981; text-transform: uppercase; font-weight: 700; white-space: nowrap;">TP3 (ATRÃ—4)</div>
                    ...
                    <div style="padding: 20px; background: var(--bg-tertiary); border-radius: 12px; text-align: center;">
                        <div style="font-size: 16px; font-weight: 700; color: #f59e0b; margin-bottom: 8px;">âš ï¸ NÃO RECOMENDADO OPERAR</div>
                ...
                <!-- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• V5 NEW PANELS â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• -->
                    ...
                    <div style="padding: 14px; background: ${analysis.btcAlignment.alignment === 'ALIGNED' ? 'rgba(34,197,94,0.06)' : analysis.btcAlignment.alignment === 'DIVERGING' ? 'rgba(239,68,68,0.06)' : 'var(--bg-tertiary)'}; border-radius: 12px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                            <div>
                                <div style="font-size: 16px; font-weight: 800; color: ${analysis.btcAlignment.alignment === 'ALIGNED' ? '#22c55e' : analysis.btcAlignment.alignment === 'DIVERGING' ? '#ef4444' : '#94a3b8'};">
                                    ${analysis.btcAlignment.alignment === 'ALIGNED' ? 'âœ… ALINHADO' : analysis.btcAlignment.alignment === 'DIVERGING' ? 'âš ï¸ DIVERGENTE' : '⚖️ NEUTRO'}
                                </div>
                        ...
                        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px;">
                            <div style="background: var(--bg-card); padding: 8px; border-radius: 8px; text-align: center;">
                                ...
                                <div style="font-size: 12px; font-weight: 700; color: ${analysis.btcAlignment.btcTrend === 'UP' ? '#22c55e' : analysis.btcAlignment.btcTrend === 'DOWN' ? '#ef4444' : '#f59e0b'};">${analysis.btcAlignment.btcTrend === 'UP' ? 'â–² Alta' : analysis.btcAlignment.btcTrend === 'DOWN' ? '▼ Baixa' : '— Lateral'}</div>
                            </div>
                            <div style="background: var(--bg-card); padding: 8px; border-radius: 8px; text-align: center;">
                            ...
                            <div style="background: var(--bg-card); padding: 8px; border-radius: 8px; text-align: center;">
                                ...
                                <div style="font-size: 12px; font-weight: 700; color: ${analysis.btcAlignment.riskLevel === 'HIGH' ? '#ef4444' : analysis.btcAlignment.riskLevel === 'MEDIUM' ? '#f59e0b' : '#22c55e'};">${analysis.btcAlignment.riskLevel === 'HIGH' ? 'ðŸ”´ Alto' : analysis.btcAlignment.riskLevel === 'MEDIUM' ? 'ðŸŸ¡ Médio' : 'ðŸŸ¢ Baixo'}</div>
                            </div>
                        </div>
                        ...
                        ${analysis.mtfAnalysis.timeframes.map(tf => `
                            <div style="background: var(--bg-tertiary); padding: 8px; border-radius: 8px; text-align: center; border-top: 2px solid ${tf.trend === 'BULLISH' ? '#22c55e' : tf.trend === 'BEARISH' ? '#ef4444' : tf.trend === 'WEAK_BULL' ? '#86efac' : tf.trend === 'WEAK_BEAR' ? '#fca5a5' : '#64748b'};">
                                ...
                                <div style="font-size: 13px; font-weight: 800; margin: 4px 0; color: ${tf.trend.includes('BULL') ? '#22c55e' : tf.trend.includes('BEAR') ? '#ef4444' : 'var(--text-secondary)'};">
                                    ${tf.trend.includes('BULL') ? 'â–²' : tf.trend.includes('BEAR') ? '▼' : 'â”€'}
                                </div>
                    ...
                    <div style="padding: 14px; background: ${analysis.setupStats.quality === 'EXCELENTE' ? 'rgba(34,197,94,0.06)' : analysis.setupStats.quality === 'BOM' ? 'rgba(59,130,246,0.06)' : analysis.setupStats.quality === 'MÉDIO' ? 'rgba(245,158,11,0.06)' : 'rgba(239,68,68,0.06)'}; border-radius: 12px; border: 1px solid ${analysis.setupStats.quality === 'EXCELENTE' ? 'rgba(34,197,94,0.2)' : analysis.setupStats.quality === 'BOM' ? 'rgba(59,130,246,0.2)' : analysis.setupStats.quality === 'MÉDIO' ? 'rgba(245,158,11,0.2)' : 'rgba(239,68,68,0.2)'};">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                            <span style="font-size: 14px; font-weight: 800; color: ${analysis.setupStats.quality === 'EXCELENTE' ? '#22c55e' : analysis.setupStats.quality === 'BOM' ? '#3b82f6' : analysis.setupStats.quality === 'MÉDIO' ? '#f59e0b' : '#ef4444'};">
                                ${analysis.setupStats.quality === 'EXCELENTE' ? 'ðŸ†' : analysis.setupStats.quality === 'BOM' ? 'ðŸ‘' : analysis.setupStats.quality === 'MÉDIO' ? 'ðŸ¤”' : 'âš ï¸'} ${analysis.setupStats.quality}
                            </span>
                        ...
                        <div style="margin-top: 8px; padding: 6px 10px; background: rgba(245,158,11,0.1); border-radius: 6px; font-size: 10px; color: #f59e0b; text-align: center;">
                            âš ï¸ Amostra pequena — dados estatisticamente insuficientes
                        </div>
                        ` : ''}
                    </div>
                </div>
                ...
                <div class="ta-section" style="border: 1px solid ${analysis.marketBreadth.alignment === 'STRONG_ALIGNED' ? 'rgba(34,197,94,0.3)' : analysis.marketBreadth.alignment === 'DIVERGING' ? 'rgba(239,68,68,0.3)' : 'rgba(100,116,139,0.2)'};">
                    ...
                    <div style="padding: 14px; border-radius: 12px; background: var(--bg-tertiary);">
                        ...
                        <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                            <span style="font-size: 11px; color: #22c55e; font-weight: 700;">ðŸŸ¢ Long ${analysis.marketBreadth.longPct}%</span>
                            <span style="font-size: 11px; color: #64748b; font-weight: 600;">Neutro ${analysis.marketBreadth.neutralPct}%</span>
                            <span style="font-size: 11px; color: #ef4444; font-weight: 700;">ðŸ”´ Short ${analysis.marketBreadth.shortPct}%</span>
                        </div>
                    ...
                    <div style="padding: 14px; background: var(--bg-tertiary); border-radius: 12px;">
                        ...
                        <div style="margin-bottom: 10px;">
                            ...
                            <div style="display: flex; justify-content: space-between; margin-top: 4px; font-size: 9px; color: var(--text-muted);">
                                <span>0</span>
                                <span style="color: ${indicators.pineV7.score >= 90 ? '#22c55e' : indicators.pineV7.score >= 75 ? '#f59e0b' : '#94a3b8'}; font-weight: 700;">${indicators.pineV7.score >= 90 ? 'ðŸŸ¢ Alta Assertividade' : indicators.pineV7.score >= 75 ? 'ðŸŸ¡ Assertividade Moderada' : '⚪ Sem Setup'}</span>
                                <span>100</span>
                            </div>
                        </div>
                        ...
                        <div style="display: flex; gap: 4px; flex-wrap: wrap;">
                            <span style="font-size: 10px; padding: 3px 8px; border-radius: 6px; background: ${(indicators.pineV7.components.alta || indicators.pineV7.components.baixa) ? (indicators.pineV7.components.alta ? 'rgba(34,197,94,0.15)' : 'rgba(239,68,68,0.15)') : 'rgba(148,163,184,0.1)'}; color: ${(indicators.pineV7.components.alta || indicators.pineV7.components.baixa) ? (indicators.pineV7.components.alta ? '#4ade80' : '#f87171') : 'var(--text-muted)'};">
                                ${indicators.pineV7.components.alta ? 'â–² EMAs Alta' : indicators.pineV7.components.baixa ? '▼ EMAs Baixa' : 'â”€ Lateral'}
                            </span>
                            <span style="font-size: 10px; padding: 3px 8px; border-radius: 6px; background: ${indicators.pineV7.components.forca ? 'rgba(59,130,246,0.15)' : 'rgba(148,163,184,0.1)'}; color: ${indicators.pineV7.components.forca ? '#60a5fa' : 'var(--text-muted)'};">
                                ${indicators.pineV7.components.forca ? 'ðŸ’ª Força (ADX>' : '😴 Fraco (ADX<'}20)
                            </span>
                            <span style="font-size: 10px; padding: 3px 8px; border-radius: 6px; background: ${(indicators.pineV7.components.pullbackCompra || indicators.pineV7.components.pullbackVenda) ? 'rgba(168,85,247,0.15)' : 'rgba(148,163,184,0.1)'}; color: ${(indicators.pineV7.components.pullbackCompra || indicators.pineV7.components.pullbackVenda) ? '#c084fc' : 'var(--text-muted)'};">
                                ${(indicators.pineV7.components.pullbackCompra || indicators.pineV7.components.pullbackVenda) ? '✓ No Pullback' : 'âœ— Longe das EMAs'}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="ta-section">
                    ...
                    <div style="padding: 16px; background: linear-gradient(135deg, ${marketRegime.regimeColor}15 0%, transparent 100%); border-radius: 12px; border: 1px solid ${marketRegime.regimeColor}30;">
                        ...
                        <div style="padding: 10px; background: var(--bg-tertiary); border-radius: 8px;">
                            <div style="font-size: 11px; color: var(--text-muted);">ðŸ’¡ Implicação:</div>
                    ...
                    <div style="padding: 16px; background: var(--bg-tertiary); border-radius: 12px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                            <div style="font-size: 16px; font-weight: 800; white-space: nowrap; color: ${marketStructure.overallStructure === 'BULLISH' ? '#22c55e' : marketStructure.overallStructure === 'BEARISH' ? '#ef4444' : '#94a3b8'};">
                                ${marketStructure.overallStructure === 'BULLISH' ? 'â–²' : marketStructure.overallStructure === 'BEARISH' ? '▼' : '⚖️'} ${marketStructure.overallStructure}
                            </div>
                    ...
                    <div style="padding: 16px; background: var(--bg-tertiary); border-radius: 12px;">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px;">
                            ...
                            <div style="padding: 8px; background: ${cvdAdvanced.absorption ? 'rgba(139,92,246,0.15)' : 'var(--bg-card)'}; border-radius: 8px; text-align: center;">
                                ...
                                <div style="font-size: 14px;">${cvdAdvanced.absorption ? 'ðŸ‹' : '—'}</div>
                            ...
                            <div style="padding: 8px; background: ${cvdAdvanced.breakout ? 'rgba(249,115,22,0.15)' : 'var(--bg-card)'}; border-radius: 8px; text-align: center;">
                                ...
                                <div style="font-size: 14px;">${cvdAdvanced.breakout ? 'ðŸ’¥' : '—'}</div>
                ...
                <div class="ta-section">
                    ...
                    <div style="padding: 16px; background: var(--bg-tertiary); border-radius: 12px;">
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                            <div>
                                <div style="font-size: 14px; font-weight: 700; color: ${macroNews.macroColor}; white-space: nowrap;">
                                    ${macroNews.macroSentiment === 'BULLISH' ? 'ðŸŸ¢' : macroNews.macroSentiment === 'BEARISH' ? 'ðŸ”´' : macroNews.macroSentiment === 'CAUTELA' ? 'ðŸŸ¡' : '⚪'} 
                        ...
                        <div style="margin-bottom: 12px;">
                            <div style="font-size: 11px; color: var(--text-muted); font-weight: 700; margin-bottom: 8px; text-transform: uppercase;">ðŸ“… Próximos Eventos de Alto Impacto</div>
                            ...
                            <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px; background: var(--bg-card); border-radius: 6px; margin-bottom: 4px; border-left: 3px solid ${e.isCritical ? '#ef4444' : '#f59e0b'}; gap: 6px;">
                                <div style="min-width: 0; flex: 1; overflow: hidden;">
                                    ...
                                    <div style="font-size: 9px; color: var(--text-muted); white-space: nowrap;">${e.country || ''} â€¢ ${new Date(e.date).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</div>
                                </div>
                                <span style="font-size: 9px; padding: 2px 6px; background: ${e.isCritical ? 'rgba(239,68,68,0.2)' : 'rgba(245,158,11,0.2)'}; color: ${e.isCritical ? '#ef4444' : '#f59e0b'}; border-radius: 4px; font-weight: 600; white-space: nowrap; flex-shrink: 0;">${e.isCritical ? 'CRÃTICO' : 'ALTO'}</span>
                            </div>
                            `).join('')}
                        </div>
                        ` : ''}
                        ...
                        <div>
                            <div style="font-size: 11px; color: var(--text-muted); font-weight: 700; margin-bottom: 8px; text-transform: uppercase;">ðŸ“° Notícias de Alto Impacto</div>
                            ...
                            <div style="padding: 8px; background: var(--bg-card); border-radius: 6px; margin-bottom: 4px; border-left: 3px solid ${n.sentiment > 0 ? '#22c55e' : n.sentiment < 0 ? '#ef4444' : '#94a3b8'};">
                                <div style="font-size: 11px; color: var(--text-primary); line-height: 1.3;">${n.isUrgent ? 'ðŸš¨ ' : ''}${n.title?.substring(0, 80)}</div>
                                <div style="font-size: 9px; color: var(--text-muted); margin-top: 4px;">${n.source} â€¢ ${n.sentiment > 0 ? 'ðŸŸ¢ Bullish' : n.sentiment < 0 ? 'ðŸ”´ Bearish' : '⚪ Neutro'}</div>
                            </div>
                            `).join('')}
                        </div>
                        ` : `
                        <div style="text-align: center; padding: 12px; color: var(--text-muted); font-size: 11px;">
                            âœ… Nenhuma notícia urgente detectada
                        </div>
                        `}
                    </div>
                </div>
                ...
                <div class="ta-section">
                    ...
                    <div style="padding: 12px; background: var(--bg-tertiary); border-radius: 12px; overflow: hidden;">
                        ...
                        <div style="font-size: 11px; color: var(--text-muted); font-weight: 700; margin-bottom: 10px; text-transform: uppercase;">ðŸ¢ Big Tech — Resultados em Tempo Real</div>
                        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 6px; margin-bottom: 16px;">
                            ...
                            <div style="background: var(--bg-card); border-radius: 10px; padding: 10px 6px; text-align: center; border: 1px solid ${t.changePercent > 0 ? 'rgba(34,197,94,0.2)' : t.changePercent < 0 ? 'rgba(239,68,68,0.2)' : 'var(--border-subtle)'};">
                                ...
                                <div style="font-size: 11px; font-weight: 700; white-space: nowrap; color: ${t.changePercent > 0 ? '#22c55e' : t.changePercent < 0 ? '#ef4444' : '#94a3b8'};">
                                    ${t.changePercent > 0 ? 'â–²' : t.changePercent < 0 ? '▼' : 'â€“'} ${Math.abs(t.changePercent || 0).toFixed(2)}%
                                </div>
                            </div>
                            `).join('')}
                        </div>
                        ...
                        <div style="font-size: 11px; color: var(--text-muted); font-weight: 700; margin-bottom: 10px; text-transform: uppercase;">📊 Ãndices de Mercado</div>
                        ...
                        <div style="font-size: 11px; color: var(--text-muted); font-weight: 700; margin-bottom: 10px; text-transform: uppercase;">ðŸŽ­ Fear & Greed Index</div>
                        ...
                        <div style="font-size: 11px; color: var(--text-muted); font-weight: 700; margin-bottom: 10px; text-transform: uppercase;">ðŸ›ï¸ Treasury Yields (Títulos do Tesouro EUA)</div>
                        ...
                        <div style="padding: 8px 12px; background: rgba(239,68,68,0.12); border: 1px solid rgba(239,68,68,0.25); border-radius: 8px; margin-bottom: 16px;">
                            <div style="font-size: 11px; font-weight: 700; color: #ef4444;">âš ï¸ Curva de Juros INVERTIDA (3M-10Y: ${bigTechMacro.treasuryYields.yieldCurveSpread?.toFixed(2) || '?'}%) — Sinal de recessão</div>
                        </div>
                        ` : `
                        <div style="padding: 8px 12px; background: rgba(34,197,94,0.08); border-radius: 8px; margin-bottom: 16px;">
                            <div style="font-size: 11px; color: var(--text-muted);">âœ… Spread 3M-10Y: ${bigTechMacro.treasuryYields.yieldCurveSpread != null ? bigTechMacro.treasuryYields.yieldCurveSpread.toFixed(2) + '%' : 'N/A'} — Curva normal</div>
                        </div>
                        `}
                        ` : ''}
                        ...
                        <div style="font-size: 11px; color: var(--text-muted); font-weight: 700; margin-bottom: 10px; text-transform: uppercase;">ðŸ‡ºðŸ‡¸ Indicadores MacroeconÃ´micos dos EUA</div>
                ...
                <div class="ta-section">
                    ...
                    <div style="background: var(--bg-tertiary); border-radius: 12px; padding: 16px; margin-bottom: 16px;">
                        ...
                        <div style="margin-top: 12px; padding: 10px; background: ${parseFloat(indicators.orderFlow.fundingRate) > 0.03 ? 'rgba(239, 68, 68, 0.15)' : parseFloat(indicators.orderFlow.fundingRate) < -0.01 ? 'rgba(34, 197, 94, 0.15)' : 'rgba(245, 158, 11, 0.15)'}; border-radius: 8px;">
                            <div style="font-size: 11px; font-weight: 600; color: ${parseFloat(indicators.orderFlow.fundingRate) > 0.03 ? '#ef4444' : parseFloat(indicators.orderFlow.fundingRate) < -0.01 ? '#22c55e' : '#f59e0b'};">
                                ${parseFloat(indicators.orderFlow.fundingRate) > 0.05 ? 'âš ï¸ MUITO ALTO - Mercado sobrecomprado, pressão de venda iminente' : parseFloat(indicators.orderFlow.fundingRate) > 0.03 ? 'â–² ALTO - Longs dominantes, risco de liquidação cascata' : parseFloat(indicators.orderFlow.fundingRate) > 0.01 ? '📊 LEVEMENTE ALTO - Longs pagando shorts' : parseFloat(indicators.orderFlow.fundingRate) < -0.03 ? 'âš ï¸ MUITO NEGATIVO - Shorts dominantes, possível squeeze' : parseFloat(indicators.orderFlow.fundingRate) < -0.01 ? '▼ NEGATIVO - Shorts pagando longs' : '⚖️ NEUTRO - Mercado equilibrado'}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="ta-indicator-grid">
                        <div class="ta-indicator-item">
                            ...
                            <div class="ta-indicator-change">${indicators.orderFlow.cvdSignal.includes('absorption') ? 'âš¡ Absorção detectada' : 'Delta de volume'}</div>
                        </div>
                        <div class="ta-indicator-item">
                            <div class="ta-indicator-header">
                ...
                <div class="ta-section">
                    ...
                    <div class="ta-indicator-grid">
                        <div class="ta-indicator-item">
                            ...
                            <div class="ta-indicator-change">${parseFloat(indicators.sentiment.rsi) < 30 ? 'âš¡ Oportunidade' : parseFloat(indicators.sentiment.rsi) > 70 ? 'âš ï¸ Cuidado' : 'Normal'}</div>
                        </div>
                        <div class="ta-indicator-item">
                            <div class="ta-indicator-header">
                            ...
                            <div class="ta-indicator-change">${parseFloat(indicators.sentiment.takerRatio) > 1 ? 'ðŸŸ¢ Pressão compradora' : parseFloat(indicators.sentiment.takerRatio) < 1 ? 'ðŸ”´ Pressão vendedora' : 'Equilibrado'}</div>
                        </div>
                    </div>
                </div>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 6px; margin-top: 10px;">
                        <div style="background: linear-gradient(135deg, rgba(59, 130, 246, 0.15) 0%, transparent 100%); padding: 10px 8px; border-radius: 10px; border: 1px solid rgba(59, 130, 246, 0.3); text-align: center; min-width: 0; overflow: hidden;">
                            ...
                            <div style="font-size: 9px; color: ${parseFloat(indicators.movingAverages?.currentPrice || 0) > parseFloat(indicators.movingAverages?.ema9 || 0) ? '#22c55e' : '#ef4444'}; margin-top: 2px;">
                                ${parseFloat(indicators.movingAverages?.currentPrice || 0) > parseFloat(indicators.movingAverages?.ema9 || 0) ? 'â–² Acima' : '▼ Abaixo'}
                            </div>
                        </div>
                        <div style="background: linear-gradient(135deg, rgba(34, 197, 94, 0.15) 0%, transparent 100%); padding: 10px 8px; border-radius: 10px; border: 1px solid rgba(34, 197, 94, 0.3); text-align: center; min-width: 0; overflow: hidden;">
                            ...
                            <div style="font-size: 9px; color: ${parseFloat(indicators.movingAverages?.currentPrice || 0) > parseFloat(indicators.movingAverages?.ema21 || 0) ? '#22c55e' : '#ef4444'}; margin-top: 2px;">
                                ${parseFloat(indicators.movingAverages?.currentPrice || 0) > parseFloat(indicators.movingAverages?.ema21 || 0) ? 'â–² Acima' : '▼ Abaixo'}
                            </div>
                        </div>
                        <div style="background: linear-gradient(135deg, rgba(249, 115, 22, 0.15) 0%, transparent 100%); padding: 10px 8px; border-radius: 10px; border: 1px solid rgba(249, 115, 22, 0.3); text-align: center; min-width: 0; overflow: hidden;">
                            ...
                            <div style="font-size: 9px; color: ${parseFloat(indicators.movingAverages?.currentPrice || 0) > parseFloat(indicators.movingAverages?.ema50 || 0) ? '#22c55e' : '#ef4444'}; margin-top: 2px;">
                                ${parseFloat(indicators.movingAverages?.currentPrice || 0) > parseFloat(indicators.movingAverages?.ema50 || 0) ? 'â–² Acima' : '▼ Abaixo'}
                            </div>
                        </div>
                        <div style="background: var(--bg-tertiary); padding: 10px 8px; border-radius: 10px; text-align: center; min-width: 0; overflow: hidden;">
                            ...
                            <div style="font-size: 9px; color: ${parseFloat(indicators.movingAverages?.currentPrice || 0) > parseFloat(indicators.movingAverages?.sma50 || 0) ? '#22c55e' : '#ef4444'}; margin-top: 2px;">
                                ${parseFloat(indicators.movingAverages?.currentPrice || 0) > parseFloat(indicators.movingAverages?.sma50 || 0) ? 'â–² Acima' : '▼ Abaixo'}
                            </div>
                        </div>
                        <div style="background: var(--bg-tertiary); padding: 10px 8px; border-radius: 10px; text-align: center; min-width: 0; overflow: hidden;">
                            ...
                            <div style="font-size: 9px; color: ${parseFloat(indicators.movingAverages?.currentPrice || 0) > parseFloat(indicators.movingAverages?.sma99 || 0) ? '#22c55e' : '#ef4444'}; margin-top: 2px;">
                                ${parseFloat(indicators.movingAverages?.currentPrice || 0) > parseFloat(indicators.movingAverages?.sma99 || 0) ? 'â–² Acima' : '▼ Abaixo'}
                            </div>
                        </div>
                        <div style="background: var(--bg-tertiary); padding: 10px 8px; border-radius: 10px; text-align: center; min-width: 0; overflow: hidden;">
                            ...
                            <div style="font-size: 9px; color: ${parseFloat(indicators.movingAverages?.currentPrice || 0) > parseFloat(indicators.movingAverages?.sma200 || 0) ? '#22c55e' : '#ef4444'}; margin-top: 2px;">
                                ${parseFloat(indicators.movingAverages?.currentPrice || 0) > parseFloat(indicators.movingAverages?.sma200 || 0) ? 'â–² Acima' : '▼ Abaixo'}
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="margin-top: 12px; padding: 12px; background: var(--bg-tertiary); border-radius: 10px;">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                            ...
                            <div style="background: var(--bg-card); padding: 10px; border-radius: 8px; border-left: 3px solid #ef4444;">
                                <div style="font-size: 10px; color: var(--text-muted);">RESISTÃŠNCIA</div>
                ...
                <div class="ta-section">
                    <div class="ta-section-header">
                        <div class="ta-section-icon" style="background: linear-gradient(135deg, #ef4444 0%, #991b1b 100%);">
                            <span style="font-size: 18px;">${analysis.crashState.override.icon || 'ðŸš¨'}</span>
                        </div>
                        <div>
                    ...
                    <div style="padding: 16px; background: ${analysis.crashState.isCrash || analysis.crashState.isRapidPump ? 'rgba(239,68,68,0.1)' : 'rgba(234,179,8,0.1)'}; border-radius: 12px; border: 1px solid ${analysis.crashState.isCrash || analysis.crashState.isRapidPump ? 'rgba(239,68,68,0.3)' : 'rgba(234,179,8,0.3)'};">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                            ...
                            <span style="font-size: 11px; white-space: nowrap; color: var(--text-muted);">Direção: ${analysis.crashState.direction === 'down' ? 'â†“ Queda' : 'â†‘ Alta'}</span>
                        </div>
                    ...
                    <div style="padding: 16px; background: rgba(14,165,233,0.08); border-radius: 12px; border: 1px solid rgba(14,165,233,0.2);">
                        ...
                        <div style="text-align: center; margin-bottom: 14px;">
                            ...
                            <div style="display: inline-block; margin-top: 8px; padding: 4px 12px; border-radius: 8px; background: ${analysis.positionSize.riskLevel === 'CONSERVADOR' ? 'rgba(34,197,94,0.2)' : analysis.positionSize.riskLevel === 'MODERADO' ? 'rgba(234,179,8,0.2)' : 'rgba(239,68,68,0.2)'}; color: ${analysis.positionSize.riskLevel === 'CONSERVADOR' ? '#22c55e' : analysis.positionSize.riskLevel === 'MODERADO' ? '#eab308' : '#ef4444'}; font-size: 10px; font-weight: 700;">
                                ${analysis.positionSize.riskLevel === 'CONSERVADOR' ? 'ðŸ›¡ï¸ Risco Baixo' : analysis.positionSize.riskLevel === 'MODERADO' ? '⚖️ Risco Médio' : analysis.positionSize.riskLevel === 'AGRESSIVO' ? 'ðŸ”¥ Risco Alto' : 'âš ï¸ Risco Máximo'}
                            </div>
                        </div>
                        <!-- Explicação simples -->
                        <div style="display: flex; flex-direction: column; gap: 6px;">
                            ...
                            <div style="padding: 10px; background: rgba(239,68,68,0.1); border-radius: 8px; border: 1px solid rgba(239,68,68,0.2);">
                                <span style="font-size: 11px; color: #ef4444; font-weight: 600;">âš ï¸ Mercado em queda detectado — posição reduzida automaticamente</span>
                            </div>
                            ` : ''}
                        </div>
                        <div style="margin-top: 10px; padding: 10px; background: rgba(14,165,233,0.06); border-radius: 8px; border: 1px dashed rgba(14,165,233,0.2);">
                            <div style="font-size: 10px; color: var(--text-muted); text-align: center; line-height: 1.5;">
                                ðŸ’¡ <strong>Dica:</strong> Se você tem R$ 1.000, use no máximo R$ ${(10 * analysis.positionSize.sizePercent).toFixed(0)} nesta operação.
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div style="display: flex; flex-direction: column; gap: 6px;">
                        ${analysis.v4Gates.map(g => `
                            <div style="display: flex; align-items: flex-start; gap: 8px; padding: 8px 10px; background: ${g.passed ? 'rgba(34,197,94,0.06)' : 'rgba(239,68,68,0.06)'}; border-radius: 8px; border-left: 3px solid ${g.passed ? '#22c55e' : '#ef4444'};">
                                <span style="font-size: 14px; min-width: 20px;">${g.passed ? 'âœ…' : 'âŒ'}</span>
                                <div style="flex: 1;">
                                    <div style="font-size: 11px; font-weight: 700; color: var(--text-primary); white-space: nowrap;">${g.name}</div>
                                    <div style="font-size: 10px; color: var(--text-secondary); margin-top: 2px; line-height: 1.3;">${(g.description || '').replace(/^[âœ…âŒâš ï¸]\s?/, '')}</div>
                                </div>
                            </div>
                        `).join('')}
                    </div>
                ...
                <div class="ta-section">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 8px;">
                        ...
                        <div style="padding: 10px; background: ${analysis.volumeExpansion?.expanding ? 'rgba(34,197,94,0.1)' : 'rgba(100,116,139,0.1)'}; border-radius: 8px; border: 1px solid ${analysis.volumeExpansion?.expanding ? 'rgba(34,197,94,0.2)' : 'rgba(100,116,139,0.15)'};">
                            ...
                            <div style="font-size: 10px; color: var(--text-secondary); margin-top: 2px;">Intensidade: ${analysis.volumeExpansion?.volZScore != null ? analysis.volumeExpansion.volZScore.toFixed(2) : (analysis.volumeExpansion?.['1h']?.ratio || '?') + 'Ã— média'}${analysis.volumeExpansion?.sustained ? ' (sustentado)' : ''}</div>
                        </div>
                    </div>
                ...
                <div class="ta-section">
                    ...
                    <div style="padding: 8px 10px; background: rgba(34,197,94,0.1); border-radius: 8px; margin-top: 6px; border: 1px solid rgba(34,197,94,0.2);">
                        <div style="font-size: 11px; font-weight: 700; color: #22c55e;">âœ… ${analysis.retest.details}</div>
                ...
                <div class="ta-section" style="border: 1px solid ${analysis.sessionContext.session === 'KILL_ZONE' ? 'rgba(239,68,68,0.3)' : analysis.sessionContext.session === 'LONDON_OPEN' ? 'rgba(59,130,246,0.3)' : 'rgba(100,116,139,0.2)'}; background: linear-gradient(135deg, ${analysis.sessionContext.session === 'KILL_ZONE' ? 'rgba(239,68,68,0.05)' : analysis.sessionContext.session === 'LONDON_OPEN' ? 'rgba(59,130,246,0.05)' : 'rgba(100,116,139,0.03)'} 0%, transparent 100%);">
                    <div class="ta-section-header">
                        <div class="ta-section-icon" style="background: linear-gradient(135deg, ${analysis.sessionContext.session === 'KILL_ZONE' ? '#ef4444, #dc2626' : analysis.sessionContext.session === 'LONDON_OPEN' ? '#3b82f6, #2563eb' : '#64748b, #475569'});">
                            <span style="font-size: 18px;">${analysis.sessionContext.session === 'KILL_ZONE' ? 'ðŸ”¥' : analysis.sessionContext.session === 'LONDON_OPEN' ? 'ðŸ‡¬ðŸ‡§' : analysis.sessionContext.session === 'ASIAN' ? 'ðŸŒ' : analysis.sessionContext.isWeekend ? 'ðŸš«' : 'ðŸ•'}</span>
                        </div>
                        <div>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px;">
                        <div style="padding: 8px; background: var(--bg-primary); border-radius: 8px; text-align: center;">
                            ...
                            <div style="font-size: 14px; font-weight: 800; color: ${analysis.sessionContext.signalMultiplier >= 1.2 ? '#22c55e' : analysis.sessionContext.signalMultiplier >= 0.8 ? '#f59e0b' : '#ef4444'};">Ã—${(analysis.sessionContext.signalMultiplier || 1).toFixed(1)}</div>
                        </div>
                        <div style="padding: 8px; background: var(--bg-primary); border-radius: 8px; text-align: center;">
                    ...
                    <div style="margin-top: 8px; padding: 8px 10px; background: rgba(239,68,68,0.1); border-radius: 8px; border: 1px solid rgba(239,68,68,0.2);">
                        <div style="font-size: 11px; font-weight: 700; color: #f87171;">ðŸš« FIM DE SEMANA — Todos sinais limitados a AGUARDAR</div>
                    </div>
                    ` : ''}
                </div>
                ` : ''}
                ...
                <div class="ta-section" style="border: 1px solid ${analysis.riskEngine.killSwitchActive ? 'rgba(239,68,68,0.4)' : 'rgba(245,158,11,0.2)'}; background: ${analysis.riskEngine.killSwitchActive ? 'linear-gradient(135deg, rgba(239,68,68,0.08) 0%, transparent 100%)' : 'none'};">
                    <div class="ta-section-header">
                        <div class="ta-section-icon" style="background: linear-gradient(135deg, ${analysis.riskEngine.killSwitchActive ? '#ef4444, #dc2626' : '#f59e0b, #d97706'});">
                            <span style="font-size: 18px;">${analysis.riskEngine.killSwitchActive ? 'ðŸ›‘' : 'ðŸ›¡ï¸'}</span>
                        </div>
                        <div>
                            <div class="ta-section-title">Gestão de Risco</div>
                            <div class="ta-section-subtitle">${analysis.riskEngine.killSwitchActive ? 'âš ï¸ Kill Switch ATIVO — ' + (analysis.riskEngine.killSwitchReason || 'pausa forçada') : 'Gestão dinâmica de risco'}</div>
                        </div>
                    </div>
                    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; min-width: 0;">
                        ...
                        <div style="padding: 8px; background: var(--bg-primary); border-radius: 8px; text-align: center; min-width: 0;">
                            ...
                            <div style="font-size: 14px; font-weight: 800; color: ${(analysis.riskEngine.leverage || 1) > 5 ? '#ef4444' : '#f59e0b'};">${analysis.riskEngine.leverage || 1}Ã—</div>
                        </div>
                        <div style="padding: 8px; background: var(--bg-primary); border-radius: 8px; text-align: center; min-width: 0;">
                ...
                <div class="ta-section" style="border: 1px solid rgba(100,116,139,0.15);">
                    ...
                    <div style="margin-top: 8px; padding: 8px 10px; background: rgba(239,68,68,0.06); border-radius: 8px; font-size: 11px; color: var(--text-secondary);">
                        ðŸ’¥ Liquidações (1h): <strong>${analysis.oiAnalysis.liquidations.longs} longs</strong> / <strong>${analysis.oiAnalysis.liquidations.shorts} shorts</strong> — Total: <strong>$${(analysis.oiAnalysis.liquidations.totalUSD / 1000).toFixed(0)}K</strong>
                    </div>` : ''}
                </div>
                ...
                <!-- âš ï¸ AVISO LEGAL - Botão que abre modal -->
                ...
                <div class="ta-last-update">
                    ...
                    <span>Ãšltima atualização: ${new Date(timestamp).toLocaleTimeString('pt-BR')}</span>
                    <span style="margin-left: 8px; color: var(--accent-blue);">â€¢ Atualiza a cada 5 min</span>
