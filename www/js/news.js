        ...
        function mergeNews(newItems) {
            ...
            incomingItems.forEach(item => {
                ...
                const existing = allNews.find(news => _normalizeNewsUrl(news.url) === itemUrlKey);
                if (existing) _copyBestNewsImage(existing, item);
            });

            // Criar um Set de URLs existentes para evitar duplicatas
            const existingUrls = new Set(allNews.map(n => _normalizeNewsUrl(n.url) || n.url));
        ...
        async function fetchAINews(category, minScore) {
            try {
                let url = `${NEWS_BACKEND_URL}/news?limit=220`;
        ...
        async function fetchNews() {
            ...
            if (allNews.length > 0) {
                ...
                if (newsFilter !== 'hot') {
                    const initialTranslateCount = Math.min(180, Math.max(40, allNews.length));
        ...
        // ============================================
        const ALTSEASON_CACHE_KEY = 'altseason_cache_v3';
        const ALTSEASON_CACHE_DURATION = 60 * 60 * 1000; // 1 hora
        const ALTSEASON_ERROR_CACHE_WINDOW = 24 * 60 * 60 * 1000; // manter ultimo dado real em falha
        ...
        function getMarketWorkerUrl(path) {
            const worker = String(window.APP_CONFIG?.CALENDAR_WORKER_URL || '').trim().replace(/\/+$/, '');
        ...
        function parseBlockchainCenterAltseason(text) {
            ...
            const patterns = [
                ...
                /### Altcoin Season Index[\s\S]{0,180}?(?:Altcoin Season!\s*)?(\d{1,3})\s*Bitcoin Season/i
            ];
        ...
        async function fetchBlockchainCenterAltseason() {
            ...
            const attempts = urls.map(async (url) => {
                ...
                if (value === null) throw new Error('Altseason parse failed');
                return value;
            });
        ...
        async function fetchAltseasonIndex() {
            ...
            try {
                ...
                if (Number.isFinite(blockchainCenterValue) && blockchainCenterValue > 0) {
                    ...
                    setAltseasonCache(blockchainCenterValue, lastAltseasonBtcDom || 0, {
                        source: 'blockchaincenter_reader',
                        methodology: 'top50_vs_btc_90d',
                        lookbackDays: 90
                    });
                    updateAltseasonUI(blockchainCenterValue, lastAltseasonBtcDom || 0);
                    return;
                }
            } catch (e) {}
