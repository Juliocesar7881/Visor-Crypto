// ============================================
        ...
        const TA_SHARED_CALLS_CACHE_KEY = 'vc_shared_call_history_v3';
        const TA_SHARED_CALLS_CACHE_TTL = 60 * 1000;
        const TA_SHARED_CALLS_PERSIST_TTL = 5 * 60 * 1000;
        const TA_CALLS_CLEAN_EPOCH_MS = Date.parse('2026-04-30T14:50:00Z');
        const TA_CALL_STRATEGY_VERSION = 'S3';
        ...
        function getCallHistoryForDisplay(currentSymbol) {
            ...
            const source = shared.length > 0 ? shared : local;
        ...
        function getCallHistory() {
            try {
                ...
                return Array.isArray(history)
                    ? history.filter(call => Number(call?.timestamp || call?.time || call?.id || 0) >= TA_CALLS_CLEAN_EPOCH_MS)
                    : [];
            }
            catch { return []; }
        }
        ...
        function isTechnicalCallRecordable(fullAnalysis, direction, confidence) {
            ...
            if (hasPoints && spread < 12) return false;
            if (hasPoints && normalizedConfidence < 82 && alignedCore < 4) return false;
            if (normalizedConfidence < 82 && !displacementOk && !volumeOk) return false;
            return true;
        }
        ...
        function recordCall(symbol, signal, confidence, entryPrice, crypto, fullAnalysis) {
            ...
            if (!direction) return;
            if (!isTechnicalCallRecordable(fullAnalysis, direction, normalizedConfidence)) return;
            ...
            taAutoRefreshInterval = setInterval(async () => {
                ...
                try {
                    ...
                    if (modal.classList.contains('active')) {
                        ...
                        refreshTechnicalHistoryFromDB(symbol);
                    }
                } catch (e) {
                }
            }, 300000); // 5 minutos
...
- NÃO invente dados, use APENAS os fornecidos`;
        ...
        function resolveVisorFinalTASignal(analysis, options = {}) {
            ...
            const rawDirection = isSnapshotFrozen && frozenDirection !== 'NEUTRO'
                ? frozenDirection
                : [
                    analysis?._finalSignal,
                    analysis?.v4Signal,
        ...
        function renderTechnicalAnalysis(analysis, crypto) {
            ...
            try {
                ...
                const isSnapshotFrozen = analysis?._snapshotFrozen === true;
                const frozenDirectionText = String(analysis?._finalDirection || analysis?._finalSignal || analysis?.signal || '').toUpperCase();
            ...
            const unifiedFinalSignal = resolveVisorFinalTASignal(analysis);
            ...
            analysis._finalSignalType = signalType;
            analysis._finalSignal = signal;
            analysis._finalDirection = unifiedFinalSignal.direction;
