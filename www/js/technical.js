...
let _taRawDataCache = {};
        ...
        function evaluatePineV7Model(klines5m, klines15m, currentPrice) {
            ...
            function calcModel(klines) {
                ...
                const pullbackCompra = currentPrice <= ema21 && currentPrice >= ema50;
                const pullbackVenda = currentPrice >= ema21 && currentPrice <= ema50;

                let score = 0;
                score += (alta || baixa) ? 25 : 0;
                score += forca ? 25 : 0;
                score += (Math.abs(rsi - 50) > 0.001) ? 25 : 0;
                score += (pullbackCompra || pullbackVenda) ? 25 : 0;

                const compra = alta && pullbackCompra && rsi > 50 && forca && score >= 75;
                const venda = baixa && pullbackVenda && rsi < 50 && forca && score >= 75;
        ...
        function generateTechnicalAnalysis(data, symbol) {
            ...
            if (pineV7.active) {
                ...
                if (pineMatchesSignal) {
                    confidence = Math.min(confidence + pineV7.confidenceBoost, 97);
