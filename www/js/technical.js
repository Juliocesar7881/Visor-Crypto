...
⚠️ Confiança para operação: BAIXA. Sem sinal direcional.`;
        ...
        function renderTechnicalAnalysis(analysis, crypto) {
            ...
            // ═══ SYNC AI SUMMARY WITH DEFINITIVE DISPLAY VALUES ═══
            // Regenerate AI summary using the final computed signalType & confidence
            // so the text always matches what the header shows
            const finalAiSummary = (function() {
                try {
                    let newSummary = generateAISummary(signalType, confidence, analysis.indicators || {}, taCurrentSymbol);
                    // Preserve "━━━ ANÁLISE AVANÇADA ━━━" reactive section if present
                    const reactiveMarker = '━━━ ANÁLISE AVANÇADA ━━━';
                    const origText = aiSummary || '';
                    const reactiveIdx = origText.indexOf(reactiveMarker);
                    if (reactiveIdx !== -1) {
                        newSummary += '\n\n' + origText.substring(reactiveIdx);
                    }
                    return newSummary;
                } catch (e) {
                    return aiSummary; // fallback to original if anything fails
                }
            })();
            ...
            body.innerHTML = `
                ...
                <div class="ta-ai-summary">
                    <div class="ta-ai-header">
                        ...
                        <div class="ta-ai-title">Relatório da IA</div>
                    </div>
                    <div class="ta-ai-text" style="white-space: pre-line;">${finalAiSummary}</div>
                </div>
