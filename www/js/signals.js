        ...
        const DASH_SCAN_CACHE_TTL = 10 * 60 * 1000; // 10 min stale threshold for auto-rescan
        ...
        async function dashProgressiveScan() {
            ...
            for (const [symbol, data] of cryptos) {
                if (_dashScanAbort) break;
                // Check if we're still on the dashboard section
                const dashSection = document.getElementById('dashboard');
                if (!dashSection || !dashSection.classList.contains('active')) { _dashScanAbort = true; break; }

                // Skip if we have fresh data (< 10 min)
                const existing = dashResults[symbol];
                if (existing && existing.lastScanAt && (now - existing.lastScanAt < DASH_SCAN_CACHE_TTL) && existing.confidence) {
                    continue;
                }
                ...
                if (updatedEl) {
                    updatedEl.innerHTML = `<i class="fas fa-circle-notch fa-spin" style="font-size:8px;margin-right:3px;"></i> ${data.short}... (${scanned}/${cryptos.length})`;
                }

                try {
                    let analysis = null;

                    // 1. Check TA cache first (user might have opened TA recently)
                    const cached = typeof getTACache === 'function' ? getTACache(symbol) : null;
                    if (cached && cached.analysis) {
                        analysis = cached.analysis;
                    } else {
                        // 2. Run full TA pipeline (same as HOME)
                        ...
                        // Macro + BigTech
                        try {
                            if (window.TAEngineV2 && window.TAEngineV2.fetchMacroNewsLayer) {
                                const macroData = await window.TAEngineV2.fetchMacroNewsLayer(symbol).catch(() => null);
                                if (macroData) {
                                    analysis.macroNews = macroData;
                                    if (macroData.totalImpact !== 0) {
                                        analysis.confluenceSummary.score = (parseFloat(analysis.confluenceSummary.score) + macroData.totalImpact).toFixed(1);
                                    }
                                }
                            }
                        } catch {}
                    ...
                    // Extract results — match HOME renderTechnicalAnalysis logic EXACTLY
                    ...
                    // RULE: confidence < 50 = NEUTRO (same as HOME)
                    if (confidence < 50) {
                        signal = 'NEUTRO';
                    }

                    // Sync confidence with market regime (same 70/30 blend as HOME)
                    ...
                    // Re-check NEUTRO after regime sync (same as HOME)
                    if (confidence < 50) {
                        signal = 'NEUTRO';
                    }
                    ...
                    // Save to dash TA results
                    dashResults[symbol] = result;
                    _saveDashTAResults(dashResults);

                    // Also update scan results so active signals section sees it
                    if (!scanResults[symbol]) scanResults[symbol] = {};
                    scanResults[symbol].signal = signal;
                    scanResults[symbol].confidence = confidence;
                    ...
                    if (updatedCell) {
                        updatedCell.outerHTML = _confCellHtml(symbol, data, result);
                    }

                } catch (err) {
                    // Mark cell as no-data
                    const errCell = document.getElementById(`dash-conf-${symbol}`);
                    if (errCell) {
                        errCell.outerHTML = _confCellHtml(symbol, data, {});
                    }
                }

                scanned++;

                // Delay between cryptos to avoid API rate limiting (1.5s)
                if (!_dashScanAbort && scanned < cryptos.length) {
        ...
        function dashAbortScan() { _dashScanAbort = true; }
