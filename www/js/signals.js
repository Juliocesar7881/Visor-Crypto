        ...
        async function openTechnicalAnalysis() {
            try {
                ...
                if (document.getElementById('ta-modal-body')) {
                    ...
                    document.getElementById('ta-modal-body').innerHTML = '<div style="padding:20px;color:red;text-align:center;">Erro Crítico: ' + fatalError.message + '</div><button onclick="closeTechnicalAnalysis()" style="margin:20px auto;display:block;padding:10px 20px;border-radius:10px;background:#333;color:white;border:none;">Fechar</button>';
                } else {
        ...
        const SCAN_DEDUP_MS = 60 * 60 * 1000; // 60 min dedup por crypto
        ...
        function dashRenderHistory() {
          ...
          const _pnlCache = (() => { try { return JSON.parse(localStorage.getItem('VISOR_PNL_CACHE') || '{}') } catch { return {} } })();
          
          async function evaluateCallPnL(call) {
              if (!call.price) return;
              const now = Date.now();
              const entryPrice = parseFloat(call.price);
              const limits = [
                  { label: '2H', ms: 2 * 60 * 60 * 1000, key: 'pnl2h' },
                  { label: '4H', ms: 4 * 60 * 60 * 1000, key: 'pnl4h' }
              ];
              
              let needsSave = false;
              if (!_pnlCache[call.id]) _pnlCache[call.id] = {};
              const cached = _pnlCache[call.id];

              for (const lim of limits) {
                  if (!cached[lim.key]) {
                      if (now < call.time + lim.ms) {
                          // Ainda não atingiu o tempo
                          const el = document.getElementById(`${lim.key}-${call.id}`);
                          if (el) el.innerHTML = `<span style="font-size:9px; color:var(--text-muted); border:1px solid rgba(255,255,255,0.1); padding:2px 4px; border-radius:4px;">⏳ ${lim.label}</span>`;
                      } else {
                          // Atingiu o tempo, busca a kline exata do momento na Binance
                          try {
                              const targetTime = call.time + lim.ms;
                              const res = await fetch(`https://api.binance.com/api/v3/klines?symbol=${call.symbol}&interval=1m&startTime=${targetTime}&limit=1`);
                              const data = await res.json();
                              if (data && data[0]) {
                                  const closePrice = parseFloat(data[0][4]);
                                  const pnl = call.direction === 'LONG' ? ((closePrice - entryPrice) / entryPrice) * 100 : ((entryPrice - closePrice) / entryPrice) * 100;
                                  cached[lim.key] = { price: closePrice, pct: pnl };
                                  needsSave = true;
                              }
                          } catch (e) {}
                      }
                  ...
                  if (cached[lim.key]) {
                      const el = document.getElementById(`${lim.key}-${call.id}`);
                      if (el) {
                          const pct = cached[lim.key].pct;
                          const isWin = pct > 0.2;
                          const isLoss = pct < -0.2;
                          let color = 'var(--text-muted)';
                          let text = '0.00%';
                          if (isWin) { color = '#22c55e'; text = `+${pct.toFixed(2)}%`; }
                          else if (isLoss) { color = '#ef4444'; text = `${pct.toFixed(2)}%`; }
                          el.innerHTML = `<span style="font-size:9px; color:${color}; border:1px solid ${color}40; padding:2px 4px; border-radius:4px; font-weight:700;">${lim.label} ${text} ${isWin ? '✅' : isLoss ? '❌' : '-'}</span>`;
                      }
                  }
              }

              if (needsSave) {
                  try { localStorage.setItem('VISOR_PNL_CACHE', JSON.stringify(_pnlCache)); } catch(e) {}
              }
          }
          ...
          function _dashRenderHistoryFromData(rawHistory) {
              ...
              let html = '';
              const callsToEval = history.slice(0, 100);
              callsToEval.forEach(call => {
                  ...
                  <div class="dash-history-card" onclick="dashShowCallDetail(${call.id})">
                      ...
                      <div style="flex:1; min-width:0;">
                          ...
                          <div style="font-size: 10px; color: var(--text-muted); margin-top: 2px;">${timeStr}${priceStr ? ' · Entrada: ' + priceStr : ''}</div>
                          <div style="display: flex; align-items: center; gap: 6px; margin-top: 5px;">
                              <div id="pnl2h-${call.id}"></div>
                              <div id="pnl4h-${call.id}"></div>
                          </div>
                      </div>
                      <div style="text-align: right;">
              ...
              // Avalia histórico de ganho/perda via Binance retroativo
              setTimeout(() => {
                  callsToEval.forEach(call => {
                      evaluateCallPnL(call);
                  });
              }, 50);
          }
