            ...
            try {
                ...
                if (transactions.length > 0) {
                    ...
                    whaleActivityData = {
                        ...
                        whaleTransferVolume: whaleTransferVol,
                        direction,
                ...
                // Aguardar lookups assíncronos e re-classificar se necessário
                if (_lookupQueue.length > 0 || _lookupProcessing) {
                    const waitForLookups = async () => {
                        await processLookupQueue();
                        // Se houve novos labels descobertos, reclassificar transações
                        let reclassified = false;
                        for (const tx of whaleActivityData.transactions) {
                            if (tx.flowType === 'unknown' && tx.txid && !tx.txid.startsWith('wa_')) {
                                // Tentar re-identificar com novos labels
                                const txAdapted = { vin: [], vout: [] };
                                // Re-check all addresses from original tx data
                                if (tx.fromEntity === null || tx.toEntity === null) {
                                    // From addresses
                                    if (tx._fromAddresses) {
                                        for (const addr of tx._fromAddresses) {
                                            const id = identifyAddress(addr);
                                            if (id && id.category !== 'probable' && !tx.fromEntity) {
                                                tx.fromEntity = id;
                                                reclassified = true;
                                            }
                                        }
                                    ...
                                    // To addresses  
                                    if (tx._toAddresses) {
                                        for (const addr of tx._toAddresses) {
                                            const id = identifyAddress(addr);
                                            if (id && id.category !== 'probable' && !tx.toEntity) {
                                                tx.toEntity = id;
                                                reclassified = true;
                                            }
                                        }
                                    ...
                                    if (tx.fromEntity || tx.toEntity) {
                                        if (tx.fromEntity && tx.toEntity) {
                                            tx.flowLabel = `${tx.fromEntity.name} → ${tx.toEntity.name}`;
                                            tx.entityName = tx.flowLabel;
                                            tx.entityIcon = tx.fromEntity.icon;
                                            if (tx.fromEntity.type === 'exchange') tx.flowType = 'from_exchange';
                                            else if (tx.toEntity.type === 'exchange') tx.flowType = 'to_exchange';
                                            else tx.flowType = 'whale_transfer';
                                        } else if (tx.fromEntity) {
                                            tx.flowLabel = `${tx.fromEntity.name} → ?`;
                                            tx.entityName = tx.fromEntity.name;
                                            tx.entityIcon = tx.fromEntity.icon;
                                            tx.entityType = tx.fromEntity.type;
                                            if (tx.fromEntity.type === 'exchange') tx.flowType = 'from_exchange';
                                        } else if (tx.toEntity) {
                                            tx.flowLabel = `? → ${tx.toEntity.name}`;
                                            tx.entityName = tx.toEntity.name;
                                            tx.entityIcon = tx.toEntity.icon;
                                            tx.entityType = tx.toEntity.type;
                                            if (tx.toEntity.type === 'exchange') tx.flowType = 'to_exchange';
                                        }
                                    }
                                }
                            }
                        ...
                        if (reclassified) {
                            // Recalcular volumes
                            let toEx = 0, fromEx = 0, unkVol = 0, wTransVol = 0, totalVol = 0;
                            whaleActivityData.transactions.forEach(tx => {
                                totalVol += tx.usdValue;
                                if (tx.flowType === 'to_exchange') toEx += tx.usdValue;
                                else if (tx.flowType === 'from_exchange') fromEx += tx.usdValue;
                                else if (tx.flowType === 'whale_transfer' || tx.flowType === 'exchange_transfer') wTransVol += tx.usdValue;
                                else unkVol += tx.usdValue;
                            });
                            whaleActivityData.toExchange = toEx;
                            whaleActivityData.fromExchange = fromEx;
                            whaleActivityData.unknownVolume = unkVol;
                            whaleActivityData.whaleTransferVolume = wTransVol;
                            whaleActivityData.totalVolume = totalVol;
                            if (fromEx > toEx * 1.15) whaleActivityData.direction = 'acumulando';
                            else if (toEx > fromEx * 1.15) whaleActivityData.direction = 'vendendo';
                            else whaleActivityData.direction = 'neutral';
                            renderWhaleActivityUI();
                        }
                    };
                    // Run in background - doesn't block UI
                    setTimeout(waitForLookups, 500);
                }
                ...
                while (blockHash && blocksAnalyzed < maxBlocks) {
                    try {
                        ...
                        for (const tx of (block.tx || [])) {
                            ...
                            // Incluir TODAS as transações grandes (≥ mínimo):
                            // - Endereços conhecidos: classificar normalmente
                            // - Endereços desconhecidos: contar como "não classificado" no volume
                            ...
                            if (fromEntity && toEntity) {
                                ...
                                flowLabel = `${fromEntity.name} → ?`;
                                entityName = fromEntity.name;
                                entityIcon = fromEntity.icon;
                                entityType = fromEntity.type;
                                if (fromEntity.type === 'exchange') flowType = 'from_exchange';
                            } else if (toEntity) {
                                flowLabel = `? → ${toEntity.name}`;
                                entityName = toEntity.name;
                                entityIcon = toEntity.icon;
                                entityType = toEntity.type;
                                if (toEntity.type === 'exchange') flowType = 'to_exchange';
                            } else {
                                // Transação grande sem entidade conhecida
                                // Agendar lookups assíncronos para TODOS os endereços envolvidos
                                const allAddrs = [];
                                if (txAdapted.vin) txAdapted.vin.forEach(v => { if (v.prevout?.scriptpubkey_address) allAddrs.push(v.prevout.scriptpubkey_address); });
                                if (txAdapted.vout) txAdapted.vout.forEach(v => { if (v.scriptpubkey_address) allAddrs.push(v.scriptpubkey_address); });
                                allAddrs.forEach(a => scheduleAddressLookup(a));
                                
                                // Try structural heuristics before leaving as unclassified
                                ...
                                if (structural) {
                                    ...
                                    entityType = structural.entityType;
                                } else {
                                    flowType = 'unknown';
                                    flowLabel = 'Tx Grande (Não Classif.)';
                            ...
                            // Guardar endereços para reclassificação posterior
                            const _fromAddresses = (txAdapted.vin || []).map(v => v.prevout?.scriptpubkey_address).filter(Boolean);
                            const _toAddresses = (txAdapted.vout || []).map(v => v.scriptpubkey_address).filter(Boolean);
                ...
                <div style="padding: 16px;">
                    ...
                    ` : noData ? `
                        <div style="text-align: center; padding: 20px; color: var(--text-muted);">
                            ...
                            <div>Nenhuma transação grande detectada no período</div>
                            <div style="font-size: 11px; margin-top: 6px;">Transações ≥ $50K (exchanges, baleias e não classificadas)</div>
                        </div>
                    ` : hasData ? `
                        <!-- Status Principal -->
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 16px;">
                            ...
                            <div style="text-align: center; padding: 12px; background: var(--bg-tertiary); border-radius: 10px;">
                                <div style="font-size: 18px; font-weight: 800; color: #3b82f6;">${formatVolume(data.whaleTransferVolume || 0)}</div>
                                <div style="font-size: 10px; color: var(--text-muted); margin-top: 4px;">Baleias/OTC</div>
                            </div>
                            <div style="text-align: center; padding: 12px; background: var(--bg-tertiary); border-radius: 10px;">
                                ...
                                ${data.transactions.slice(0, 2).map((tx, i) => {
                                    ...
                                    <div style="display: flex; align-items: center; padding: 10px 12px; background: var(--bg-tertiary); border-radius: 8px; margin-bottom: 6px; border-left: 3px solid ${borderColor};">
                                        <div style="flex: 1; min-width: 0;">
                                            ...
                                            ${tx.flowLabel ? `
                                                <div style="font-size: 11px; font-weight: 600; color: #3b82f6; margin-bottom: 3px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                    ...
                    <div style="padding: 10px; background: var(--bg-tertiary); border-radius: 8px; font-size: 11px; color: var(--text-muted);">
                        ...
                        <span style="color: var(--text-secondary);">📊 Transações ≥$50K • Exchanges, baleias e não classificadas</span>
