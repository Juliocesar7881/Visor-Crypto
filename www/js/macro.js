                ...
                if (event.history && event.history.length > 0) {
                    ...
                    historySection.innerHTML = `
                        ...
                        <div style="text-align: center; padding: 12px; color: var(--text-muted); font-size: 12px;">
                            <i class="fas fa-clock" style="opacity: 0.5;"></i> Dados ainda não disponíveis
                        </div>
                    `;
                }
            }
        ...
        const CALENDAR_CACHE_KEY = 'economic_calendar_cache_v2';
        const CALENDAR_CACHE_DURATION = 10 * 60 * 1000; // 10 minutos
        
        // Clear old cache key on upgrade
                ...
                if (response.ok) {
                    ...
                    if (Array.isArray(fmpData) && fmpData.length > 0) {
                        ...
                        const importantEvents = [
                            ...
                            'ecb', 'fed chair', 'treasury', 'pce', 'adp', 'jolts', 'job opening',
                            'durable goods', 'industrial production', 'michigan', 'empire state',
                            'philadelphia fed', 'personal income', 'personal spending', 'building permits',
                            'trade balance', 'capacity utilization'
                        ];
                        
                        // Traduções de eventos - títulos descritivos em Português
                        const translations = {
                            ...
                            'employment change': 'Variação de Emprego',
                        ...
                        // Categorias semânticas para agrupar eventos similares
                        // Eventos da mesma categoria em dias próximos são duplicatas semânticas
                        const semanticCategories = {
                            'employment': ['nonfarm', 'payroll', 'employment', 'adp', 'jobs', 'job opening', 'jolts', 'hiring', 'unemployment rate', 'unemployment claims', 'jobless claims', 'initial claims'],
                            'inflation_cpi': ['core cpi', 'cpi '],
                            'inflation_ppi': ['ppi', 'producer price'],
                            'inflation_pce': ['pce', 'personal consumption expenditure'],
                            'gdp': ['gdp', 'gross domestic'],
                            'fed': ['fomc', 'fed funds', 'federal reserve', 'interest rate decision'],
                            'retail': ['retail sales'],
                            'ism_mfg': ['ism manufacturing', 'ism mfg'],
                            'ism_svc': ['ism services', 'ism non-manufacturing', 'ism svc'],
                            'housing': ['housing starts', 'building permits', 'existing home', 'new home', 'home sales'],
                        ...
                        // Função para obter categoria semântica de um evento
                        function getSemanticCategory(eventName) {
                            const lower = eventName.toLowerCase();
                            for (const [cat, keywords] of Object.entries(semanticCategories)) {
                                if (keywords.some(kw => lower.includes(kw))) return cat;
                            }
                            return null;
                        }
                        
                        // Função para traduzir título - tenta matches mais específicos primeiro
                        function translateTitle(rawTitle) {
                            const lower = rawTitle.toLowerCase();
                            // Sort keys by length descending so longer (more specific) matches are tried first
                        ...
                        const filteredData = fmpData
                            ...
                            // Ordenar por data (mais próximos primeiro) e por impacto (High > Medium > Low)
                            .sort((a, b) => {
                        ...
                        // 1. Mesmo nome traduzido no mesmo dia
                        // 2. Mesma categoria semântica em dias próximos (janela 5 dias)
                        // 3. Mesmo título traduzido em dias diferentes (janela 25 dias)
                        const seenTitles = new Set();
                        const seenCategories = {}; // cat -> earliest date timestamp
                        const seenTranslated = {}; // translatedTitle -> earliest date timestamp
                        const CATEGORY_WINDOW_MS = 5 * 24 * 60 * 60 * 1000; // 5 dias
                        const TITLE_WINDOW_MS = 25 * 24 * 60 * 60 * 1000; // 25 dias
                        
                        const deduplicatedData = filteredData.filter(e => {
                            const translatedTitle = translateTitle(e.event || '');
                            const eventDate = new Date(e.date);
                            const dateStr = eventDate.toDateString();
                            
                            // Camada 1: mesmo título traduzido no mesmo dia
                            const titleKey = `${translatedTitle}-${dateStr}`;
                            if (seenTitles.has(titleKey)) return false;
                            seenTitles.add(titleKey);
                            
                            // Camada 2: mesma categoria semântica em janela de 5 dias
                            const semCat = getSemanticCategory(e.event || '');
                            if (semCat) {
                                const catKey = semCat;
                                if (seenCategories[catKey]) {
                                    const prevDate = seenCategories[catKey];
                                    if (Math.abs(eventDate.getTime() - prevDate) < CATEGORY_WINDOW_MS) {
                                        return false; // Evento similar muito próximo, pular
                                    }
                                }
                                seenCategories[catKey] = eventDate.getTime();
                            }
                            
                            // Camada 3: mesmo título traduzido em janela de 25 dias
                            if (seenTranslated[translatedTitle]) {
                                const prevDate = seenTranslated[translatedTitle];
                                if (Math.abs(eventDate.getTime() - prevDate) < TITLE_WINDOW_MS) {
                                    return false;
                                }
                            }
                            seenTranslated[translatedTitle] = eventDate.getTime();
                            
                            return true;
                        });
                        
                        events = deduplicatedData
                            .slice(0, 18) // Limitar a 18 eventos
                            .map(e => {
                                const eventDate = new Date(e.date);
                                const months = ['JAN', 'FEV', 'MAR', 'ABR', 'MAI', 'JUN', 'JUL', 'AGO', 'SET', 'OUT', 'NOV', 'DEZ'];
                                ...
                                // Traduzir nome do evento (usando função definida acima)
                                let title = translateTitle(e.event || 'Evento');
