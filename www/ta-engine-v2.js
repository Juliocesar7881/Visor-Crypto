    ...
    async function fetchMacroNewsLayer(symbol) {
        ...
        try {
            ...
            const [calendarRes, newsRes] = await Promise.all([
                fetch(`${window.APP_CONFIG.CALENDAR_WORKER_URL}/proxy/fmp/api/v3/economic_calendar?from=${fromDate}&to=${toDate}&apikey=${FMP_KEY}`)
    ...
    async function fetchBigTechAndMacro() {
        ...
        try {
            ...
            const fredFetch = (seriesId) => {
                ...
                return fetch(`${window.APP_CONFIG.CALENDAR_WORKER_URL}/proxy/fred/fred/series/observations?series_id=${seriesId}&sort_order=desc&limit=1&api_key=${FRED_KEY}&file_type=json`, { signal: AbortSignal.timeout(8000) })
